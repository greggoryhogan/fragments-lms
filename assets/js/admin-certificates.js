(function($) {
    ...
    $(document).on('click', '#preview-certificate', function(e) {
        ...
        var user = $('#course-completed-users').val();
        var user_entry_id = $('#course-completed-users').find('option:selected').attr('data-entry-id');
        var link = flms_admin_certificates.certificate_link + '/' + course + '/' + course_version + '/' + user + '/' + user_entry_id + '/';
