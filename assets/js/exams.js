(function($) {
    ...
    $(document).on('click','.exam-pagination',function(e) {
        ...
        if($(this).hasClass('previous')) {
            ...
            var question_counter = (current_exam_page - 1) * flms_exams.per_page;
        } else {
            current_exam_page += 1;
    ...
    $(document).on('click','.exam-pagination-navigation',function(e) {
        e.preventDefault();
        var pageto = $(this).attr('data-page');
        current_exam_page = pageto;
        question_counter = (pageto - 1) * flms_exams.per_page;
        reset_exam_timer = 0;
        get_paginated_exam_questions(question_counter, current_exam_page, 0, review);
        let examTop = $("#current-exam").offset().top - 50;
        window.scrollTo(0, examTop);
    ...
    $(document).on('click','#save_exam', function() {
        ...
        $('.flms-pagination').addClass('is-loading');
        $('.exam-navigation').addClass('is-loading');
        isSaving = true;
        $.ajax({
            ...
            success: function(data) {
                ...
                $('.flms-pagination').removeClass('is-loading');
                $('.exam-navigation').removeClass('is-loading');
            },
        });
    });
    ...
    function submitExam() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                progressSaved = true;
                //console.log(data);
                if(data.submission_error == 1) {
                    alert(data.unanswered_questions);
                    $('.flms-question').removeClass('is-loading');
                    $('.flms-pagination').removeClass('is-loading');
                    $('.exam-navigation').removeClass('is-loading');
                } else {
