 ...
 */
 ...
 <header class="flms-header">
	...
	do_action( 'flms_before_heading' );

	global $post;
	if ( apply_filters( 'flms_show_page_title', true, $post ) ) : 
		$title = '<h1 class="flms-header__title page-title">'.flms_get_the_title($post, 'flms-header').'</h1>';
		echo apply_filters('flms_header_title', $title);
	endif;
