...
class FLMS_Module_Groups {
        ...
        if(is_array($open_seats)) {
            if(!empty($open_seats)) {
                ...
                if($empty_seats > 0) {
                    ...
                    $return .= '<div class="flms-primary flms-secondary-bg flms-secondary-border flms-flex flms-unassigned-seats-container">';
                        $return .= "<p>You have <strong>$empty_seats</strong> unassigned $label.";
                        if(empty($groups)) {
                            $return .=' Create a '.strtolower($group_label).' to assign them.';
                        }
                        $return .= "</p>";
                        if(is_array($open_seats) && !empty($groups)) {
                                ...
                                $return .= '<form id="assign-seats-form">'; //class="flms-d-none"
                                    $return .='<div class="flms-table-layout columns-4 mb-4">';
                                        ...
                                        foreach($open_seats as $course => $data) {
                                            ...
                                            foreach($data as $order_info) {
                                                if ( get_post_status( $course_id ) !== false ) {
                                                    ...
                                                    //foreach($order_data as $order_info) {
                                                        ...
                                                        if(!empty($groups)) {
                                                            ...
                                                            foreach($groups as $group) {
                                                                ...
                                                                $return .='<option value="'.$group->ID.'">'.flms_get_the_title($group->ID).'</option>';
                                                            }
                                                            $return .='</select>';
                                                        } else {
        ...
        $return .= '<div class="my-groups-list course-list-item columns-4 mb-4 '.$group_visibility.'">';
            ...
            if(!empty($groups)) {
                foreach($groups as $group) {
                    ...
                    $tooltip = '';
                    $group_courses = get_post_meta($group->ID, 'group_courses', true);
                    if(!empty($group_courses)) {
                        $heading = flms_get_label('groups_singular').' '. flms_get_label('course_plural').':';
                        $tooltip .= '&nbsp;&nbsp;<span class="flms-tooltip is-active" data-tooltip="'.$heading;
                        $has_courses = false;
                        foreach($group_courses as $course_info => $course_settings) {
                            ...
                            if ( get_post_status( $course_id ) !== false ) {
                                ...
                                $tooltip .= '<br>'.strip_tags($course->get_course_version_name($course_version));
                            }
                        }
                        $tooltip .= '"></span>';
                        if(!$has_courses) {
                            //$tooltip .= '<br>No active '. strtolower(flms_get_label('course_plural'));
                            $tooltip = '';
                        }
                    }

                    //$return .='<pre>'.print_r($group,true).'</pre>';
                    $return .='<div data-label="Group name"><a href="'.get_permalink($group->ID).'" title="'.strip_tags(flms_get_the_title($group->ID)).'">'.flms_get_the_title($group->ID).'</a>'.$tooltip.'</div>';
    ...
    public function get_new_group_form($hidden = true, $update = false, $post_id = 0) {
        $group_label = flms_get_label('groups_singular');
        if($post_id > 0) {
            $group_name = flms_get_the_title($post_id);
        ...
        $location = 'new-group-form';
        if($update) {
            $location = 'update-group-form';
        }
        ...
        $html .= '">';
            ...
            $html .= '<div class="form-row woocommerce-form-row flms-form-row flms-group-code-inputs '.$location.'">';
                $html .= '<label class="flms-label-full-width full-flex">'.$group_label.' Code</label>';
                $code_text = apply_filters('flms_group_code_description','The code new users use to join your '.strtolower($group_label));
                $html .= '<span class="flms-mb-1 flms-d-block"">'.$code_text.'</span>';
                ...
                $html .= '<div class="flms-flex">';
                    ...
                    $html .= '<div id="flms-group-code-validator">';
                        $enabled = apply_filters('flms_group_code_editable', true);
                        $disabled = '';
                        if(!$enabled) {
                            $disabled = ' disabled';
                        }
                        $html .= '<input name="flms-group-code" id="flms-group-code" value="'.$group_code.'" type="text" placeholder="'.apply_filters('flms_group_code_placeholder', "Create a Code for your $group_label.", $group_label).'" class="flex-1" '.$disabled.' />';
                        $html .= '<div class="validity"></div>';
                    $html .= '</div>';
                    if(apply_filters('flms_show_group_code_validator', true)) {
                        $html .= '<button id="check-group-code" class="button button-primary">Validate Code</button>';
                    }
                    if(apply_filters('flms_show_group_code_generator', true)) {
    ...
    public function flms_groups_metabox() {
        ...
        $metabox_fields = array(
            ...
            'members' => array(
				...
				'description' => '',
				'tooltip' => '',
				'callback' => $this->get_group_members('standard')
			),
            'courses' => array(
				'label' => "Group Courses",
				'id' => 'group-courses',
				'description' => '',
				'tooltip' => '',
				'callback' => $this->get_group_courses('grid'),
                'layout' => 'grid'
			),
        ); ?>
    ...
    public function get_group_settings($layout) {
		...
		$return = '<div class="group-settings">';
            ...
            $return .= '<div class="settings-field">';
                $return .= '<div class="setting-field-label">';
                    $return .= 'Group Manager(s)';
                    if($layout == 'grid') {
                        $return .= '<div class="flms-tooltip" data-tooltip="Comma separated. Group managers can add and remove members but not change group settings."></div>';
                    }
                $return .= '</div>';
                if($layout != 'grid') {
                    $return .= '<p class="description">Comma separated. Group managers can add and remove members but not change group settings.</p>';
                }
                $return .= '<input name="flms-group-manager" id="flms-group-manager" value="'.$group_managers.'" type="text" />';
                //$return .= '<input type="hidden" name="flms-group-manager-id" id="flms-group-manager-id" value="'.$managers.'" />';
            $return .= '</div>';
    ...
    public function get_group_members($layout) {
        global $post;
        if(isset($post->ID)) {
            $post_id = $post->ID;
        } else {
        ...
        return $this->get_group_member_admin_content($post_id);
    }

    public function get_group_member_admin_content($post_id) {
        $current_user_id = get_current_user_id();
        $group_members = get_post_meta($post_id, 'flms_group_member');
        $group_owner = get_post_meta($post_id, 'flms_group_owner', true);
        $managers = get_post_meta($post_id, 'flms_group_manager');
        $manager_ids = array();
        if(!empty($managers)) {
            $invited = get_post_meta($post_id,'flms_manager_invites');
            $notfound = array();
            $needs_invites = false;
            foreach($managers as $manager) {
                ...
                if($user === false) {
                    ...
                    if(!empty($invited)) {
                        delete_post_meta($post_id,'flms_manager_invites',$user->user_email);
                    }
                }
            }
            
        }
        $return = '<div class="group-members">';
            
            if(empty($group_members)) {
                $return .= sprintf('<p><em>This %s has no members.</em></p>',strtolower($group_label));
            } else {
                ...
                if($group_courses != '') {
                    ...
                    $member_list = '';
                    $member_list .= '<div class="flms-table-layout course-list-item columns-3 member-row"><strong>Name</strong><strong>Last Active</strong><strong class="actions">Actions</strong></div>';
                    foreach($group_members as $group_member) {
                        $user = get_user_by('id', $group_member);
                        if($user !== false) {
                            $active_members = true;
                            $member_list .= '<div class="flms-table-layout course-list-item columns-3 member-row">';
                    ...
                    if($active_members) {
                        ...
                        $return .= sprintf('<p><em>This %s has no active members.</em></p>',strtolower($group_label));
                    }
    ...
    public function get_group_courses($layout) {
        ...
        return $this->get_group_courses_content($post_id);
        
    }

    public function get_group_courses_content($post_id) {
        global $flms_settings;
        ...
        $return = '<div class="flms-admin-group-manager">';
            ...
            $return .= '<div class="flms-flex column group-add-course-form">';
                $return .= '<div class="flms-flex column flex-1">';
                    $return .= '<div class="flms-flex flex-1">';
                        $return .= '<div class="flms-flex column flex-1">';
                            $return .= '<div class="setting-field-label">Course</div>';
                            $return .= '<input type="text" value="" placeholder="Type to search courses" id="group-course-search" class="full-width-input regular-text" data-course="-1" data-version="-1" />';
                        $return .= '</div>';
                        $return .= '<div class="flms-flex column flex-1">';
                            $return .= '<div class="setting-field-label">Seats</div>';
                            $return .= '<input type="number" value="10" id="group-course-seats" />';
                        $return .= '</div>';
                    $return .= '</div>';
                    if(flms_is_module_active('course_credits')) {
                        $return .= '<label class="customer-paid-reporting-fees-label"><input type="checkbox" id="group-course-paid-fees" checked /> Customer paid reporting fees</label>';
                    }
                $return .= '</div>';
                $return .= '<button class="button button-primary align-self-start" id="group-enroll-course" data-group="'.$post_id.'">Add Course</button>';
                $return .= '<p class="description"><em>Tip: To increase (or decrease) the number of seats for a group, add an existing course with the change in seat numbers.</em></p>';
            $return .= '</div>';

            // $return .= '<pre>'.print_r(get_post_meta($post_id,'group_courses',true),true).'</pre>';
            if(empty($group_courses)) {
                $course_label = strtolower(flms_get_label('course_plural'));
                $group_label = strtolower(flms_get_label('groups_singular'));
                $return .= "<p><em>No $course_label to show.</em></p>";
            } else {
                $return .= '<div class="flms-course-list mb-4 flms-groups-list">'; //course-name-first course-list-item columns-'.$columns.'
                    ...
                    foreach($group_courses as $course_info => $course_settings) {
                        ...
                        //check if it exists
                        if ( get_post_status( $course_id ) !== false ) {
                            $return .= '<div class="flms-course flms-course-output settings-field col-1-3">';
                                $return .= '<div class="course-info h100">';
                                    $course_version = $course_data[1];
                                    $course = new FLMS_Course($course_id);
                                    global $flms_active_version;
                                    $flms_active_version =  $course_version;
                                    ...
                                    //$return .= '</div>';
                                    $return .= '<div class="flms-group-owner-info reporting-fees flms-flex align-vertical">';
                                        ...
                                        $return .= '</div>';
                                    $return .= '</div>';
                                    $return .= '<div class="flms-group-owner-info flms-flex align-vertical">';
                                        $return .= '<div class="flms-label flms-desktop-only">Available Seats:</div><div data-label="Available Seats:*">'.$seats . ' <span class="flms-mobile-only">available</span></div>';
                                    $return .= '</div>';
                                    $return .= '<div class="flms-group-owner-info flms-flex align-vertical">';
                                        $return .= '<div class="flms-label flms-desktop-only">Enrolled:</div><div data-label="Enrolled:*">'.$enrolled . ' <span class="flms-mobile-only">enrolled</span></div>';
                                    $return .= '</div>';
                                    $return .= '<div class="admin-remove-course-from-group mt-5" data-course-index="'.$course_index.'">Remove Course</div>';
                                ...
                                $return .= '</div>'; //close course info
                                
                                $return .= '<div id="course-progress-'.$course_index.'" class="course-progress h100">';
                                    $return .= '<div class="flms-group-owner-info">';
                                        $return .= '<div class="flms-flex gap-sm">';
                                            $return .= '<input type="email" value="" class="group-enroll-user-email" placeholder="sample@example.com" />';
                                            $return .= '<button class="group-enroll-user button button-primary" data-course-index="'.$course_index.'" data-group-id=>Enroll</button>';
                                        $return .= '</div>';  
                                        $return .= '<div class="enroll-response-'.$course_index.'"></div>';      
                                    $return .= '</div>';
                                    if(!empty($enrolled_users)) {
                                        $return .= '<div class="settings-field col-2 enrolled-user-list">';
                                            $return .= '<div class="flms-font-bold flms-desktop-only">Name</div>';
                                            $return .= '<div class="flms-font-bold flms-desktop-only">Action</div>';
                                            ...
                                            foreach($enrolled_users as $enrolled_user_id) {
                                                ...
                                                if($user !== false) {
                                                    ...
                                                    $return .= '</div>';
                                                    $return .= '<div data-label="Status:" class="btn-small is-admin admin-remove-user-from-course" data-user="'.$enrolled_user_id.'" data-course-index="'.$course_index.'">Unenroll</div>';
                                                }
                                        ...
                                        $return .= '<div class="no-users"><em>No enrolled users.</em></div>';
                                    }
                                $return .= '</div>';
    ...
    public function save_settings($post_id, $data) {
        ...
        if(isset($data['flms-group-manager'])) {
            $managers = explode(',',sanitize_text_field($data['flms-group-manager']));
            delete_post_meta($post_id, 'flms_group_manager');
            foreach($managers as $manager) {
                add_post_meta($post_id, 'flms_group_manager', $manager);
            }
        }
    ...
    public function flms_groups_member_footer_content() {
        global $post;
    ...
    public function flms_groups_member_header_content() {
        global $post;
        $current_user_id = get_current_user_id();
        $current_user = get_user_by('id', $current_user_id);
        $group_owner = get_post_meta($post->ID, 'flms_group_owner', true);
                        ...
                        if(isset($v['seats'])) {
                            if($v['seats'] > 0) {
                                ...
                                if(!in_array($current_user->ID, $enrolled)) {
                                    $link = '<a href="'.get_permalink($group).'" title="'.flms_get_the_title($group).'">'.flms_get_the_title($group).'</a>';
