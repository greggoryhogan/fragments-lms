...
class FLMS_Question {
	...
	public function question_display($echo = false, $count = '', $user_answers = array(), $review = 0, $exam_is_graded = 'graded', $passed = false, $display_type = 'exam') {
		...
		if($display_type == 'print') {
			if(apply_filters('flms_disable_print_exam_inputs', false)) {
		...
		$return .= '>';
			...
			if($display_type == 'print' && $this->question_type != 'prompt') {
				$return .= '<table class="flms-question-heading-table"><tr><td width="25">'.$count.'.</td><td width="100%">'.$question_content.'</td></tr></table>';
			}
			if($display_type != 'print') {
				...
				switch($this->question_type) {
						...
						if(is_array($answer)) {
							foreach($answer as $option) {
								if($display_type != 'print' || $show_answer) {
									...
									$return .= '<table style="padding: 3px 0 0 0;"><tr><td width="45">&nbsp;</td><td width="100%">'.$option['answer'].'</td></tr></table>';
								}
							}
						}			
						break;
						...
						if(is_array($answer)) {
							foreach($answer as $option) {
								$return .= '<div class="answer-option">';
									...
									if($user_answer !== false) {
										if(in_array($option['answer'],$user_answer) && $show_answer) {
											$return .= ' checked="checked"';
										}
									}
									$return .= $disabled.'> ';
					...
					case 'fill-in-the-blank':
						...
						if($user_answer !== false && $show_answer) {
						...
						if (preg_match('/[{](.*?)[}]/', $answer, $matches) == 1) {
							...
							if(isset($matches[1])) {
								...
								foreach($matches[1] as $k => $option_input) {
									...
									if(count($defaults) > 1 && $show_answer) {
									...
									$option_text .= '<input type="radio" name="question-'.$this->question_id.'" name="question-'.$this->question_id.'-'.$option_input.'" value="'.$value.'" '.$checked.$disabled.'>';
									if($output_value) {
										$option_text .= $value;
									}
									$option_text .= '</label>';
					...
					case 'essay':
						wp_enqueue_editor();
							$return .= '<div class="answer-option"><textarea name="question-'.$this->question_id.'" id="question'.$this->question_id.'" rows="15" '.$disabled.'>';
						if($user_answer !== false && $show_answer) {
							$return .= $user_answer;
						}
						$return .= '</textarea></div>';
				...
				if($review == 1 && $exam_is_graded == 'graded' && $passed && $display_type != 'print') {
