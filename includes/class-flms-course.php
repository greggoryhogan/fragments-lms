...
class FLMS_Course {
	...
	public function get_latest_course_version() {
		global $flms_latest_version;
		return $flms_latest_version;
	}
	...
	public function get_all_course_steps() {
		...
		foreach($steps as $k => $v) {
			$k_status = get_post_status($k);
			if($k_status == 'publish') {
				$course_steps[] = $k;
			}
			foreach($v as $i => $j) {
				$j_status = get_post_status($j);
				if($j_status == 'publish') {
					$course_steps[] = $j;
				}
			}
	...
	public function get_course_version_name($course_version) {
		...
		if(isset($flms_course_version_content["$course_version"]['force_version_title_on_latest'])) {
			if($flms_course_version_content["$course_version"]['force_version_title_on_latest'] != '') {
				$force_version_for_latest = true;
			}
		}
		if(($flms_latest_version == $course_version) && !$force_version_for_latest) {
			$title = flms_get_the_title($flms_course_id);
		} else {
			if(isset($flms_course_version_content["$course_version"]['version_name'])) {
				$format = apply_filters('flms_default_version_title_format','default');
				if(isset($flms_course_version_content["$course_version"]['version_title_display'])) {
					$format = $flms_course_version_content["$course_version"]['version_title_display'];
					$format = apply_filters('flms_global_override_version_title_format',$format);
				}
				switch($format) {
					case 'hide-version-name':
						$title = flms_get_the_title($flms_course_id);
						break;
					case 'replace-title':
						$title = $flms_course_version_content["$course_version"]['version_name'];
						break;
					case 'custom':
						$title = apply_filters('flms_version_title_custom_output', flms_get_the_title($flms_course_id), $flms_course_id, $course_version, $flms_course_version_content["$course_version"]);
						break;
					default:
						$title = flms_get_the_title($flms_course_id) . ' <span class="flms-version-name">('.$flms_course_version_content["$course_version"]['version_name'].')</span>';
						break;
				}
	...
	public function get_course_version_permalink($course_version) {
		...
		global $flms_course_id, $flms_course_version_content, $flms_latest_version, $wp;
		if($flms_latest_version == $course_version || isset($wp->query_vars['course-version'])) {
			...
			if(isset($flms_course_version_content["$course_version"]['version_permalink'])) {
				$permalink = trailingslashit(trailingslashit( get_permalink( $flms_course_id)) . $flms_course_version_content["$course_version"]['version_permalink']);
			} else {
				$permalink = trailingslashit(get_permalink($flms_course_id));
			}
		}
