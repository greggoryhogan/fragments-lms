...
class FLMS_Setup {
	...
	public function __construct() {
		...
		add_filter('manage_edit-flms-question-categories_columns', array($this, 'question_category_columns'));
        add_filter('manage_flms-question-categories_custom_column', array($this, 'question_category_column'), 10, 3 );
		...
		add_action( 'flms_export_user_data', array($cron, 'export_user_data'), 10, 3 );
	...
	function question_category_columns( $columns ) {
        $new_columns = array();
        if(isset($columns['cb'])) {
    ...
    function question_category_column( $columns, $column, $id ) {
        if ( $column == 'flms_thumbnail' ) {
            ...
            $my_posts = get_posts(array(
                'post_type' => 'flms-questions', //post type
                'numberposts' => -1,
                'tax_query' => array(
                  array(
            ...
            $columns = '<a href="'.admin_url('edit.php?'.$term->taxonomy.'='.$term->slug).'&post_type=flms-questions&orderby=menu_order&order=asc">'.count($my_posts).'</a>'; 
        }
	...
	public function flms_admin_menu_items() {
		...
		add_submenu_page(FLMS_PLUGIN_SLUG,'User Acitvity','User Activity','install_plugins', 'flms-user-activity',array($this,'flms_user_activity_table'),97);
	...
	public function flms_exam_editor() {
		?><div class="wrap">
			...
			if($exam_id > 0 && $exam_version > 0 && $user_id > 0) {
				...
				if($user !== false) { 
					$user = get_user_by('id',$user_id);
					echo '<h2>'.flms_get_the_title($exam_id).'</h2>';
						...
						if(isset($wp->query_vars['course-version'])) {
							if($wp->query_vars['course-version'] != '') {
								...
								foreach($flms_course_version_content as $k => $v) {
									if($v['version_name'] == $version_name || $v['version_permalink'] == $version_name) {
										$active_version = $k;
										break;
									}
								}
	...
	public function flms_custom_column_data($column, $post_id) {
		...
		switch ( $column ) {
			case 'course' :
				...
				if($course_id != '') {
					echo '<a href="'.get_edit_post_link($course_id).'" title="'.flms_get_the_title($course_id).'">'.flms_get_the_title($course_id).'</a>';
				} else {
					echo 'Not set';
				}
				break;
			...
			case 'lesson' :
				...
				if(!empty($lesson_ids)) {
					...
					foreach($lesson_ids as $lesson) {
						...
						echo '<a href="'.get_edit_post_link($lesson_id).'&set-course-version='.$lesson_version.'" title="'.flms_get_the_title($lesson_id).'">'.flms_get_the_title($lesson_id).' Version '.$lesson_version.'</a>'; // ('.flms_get_post_type($lesson_id['post_id']).')
						$lessonct++;
					}
				} else {
					echo 'None';
				...
				if($post->post_type == 'flms-questions') {
					...
					if(!empty($terms)) {
						...
						foreach($terms as $term) {
							$terms_list[] = '<a href="'.admin_url('edit.php?flms-question-categories='.$term->slug).'&post_type=flms-questions&orderby=menu_order&order=asc">'.$term->name.'</a>';
	...
	public function admin_bar_versions($admin_bar){
		...
		if(is_array($versions)) {
			...
			$link = flms_get_course_version_permalink($course_id, $flms_active_version);
			...
			krsort($versions);
			unset($wp->query_vars['course-version']); //bug in permalink replacement
			foreach($versions as $k => $v) {
				if(isset($versions["{$k}"]['version_permalink'])) {
					...
					$new_permalink = flms_get_course_version_permalink($course_id, $k);
			...
			$wp->query_vars['course-version'] = $version;  //bug in permalink replacement
		}
		return $admin_bar;
		...
		<?php if(current_user_can('edit_posts')) { ?>
			...
			<td>
				<div class="flms-flex column">
					...
					<?php 
					if(flms_is_module_active('course_credits')) {
						echo '<label class="customer-paid-reporting-fees-label"><input type="checkbox" id="flms-enrollment-accept-reporting-fee-override" checked /> Customer paid reporting fees</label>';
					}
					?>
					<button class="profile-enroll-user button button-primary align-self-start" id="profile-enroll-user" data-user="<?php echo $user_id; ?>">Enroll</button>
				</div>
			</td>
			</tr>
		<?php } ?>
	...
	public function flms_user_activity_table() {
		$course_progress = new FLMS_Course_Progress();
		$course_progress->display_course_activity_table();
	}
