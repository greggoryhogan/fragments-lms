...
class FLMS_Course_Manager {
					...
					echo '<div class="setting-area-fields cumulative-settings-fields">';
						...
						foreach($exams as $exam) {
							...
							echo '<div class="settings-field">';
								echo '<label>'.flms_get_the_title($exam).'</label>';
	...
	public function flms_course_metabox() {
		...
		if(flms_is_module_active('course_tabs')) {
			...
			if(!empty($tabs)) {
				foreach($tabs as $k => $v) {
					$metabox_fields['course-tab-'.$k] = array(
						'label' => $v['label'],
						'id' => 'course-tab-'.$v['key'],
						'description' => '',
						'tooltip' => '',
						'callback' => $course_tabs->output_course_tab_wysiwyg($v['key'])
					);	
				}
			}
		}
		...
		$return = '<div class="course-settings">';
			...
			if(flms_is_module_active('course_certificates')) {
				...
				$return .= '<div class="settings-field">';
					...
					//$return .= '<pre>'.print_r($versions["$active_version"], true).'</pre>';
	...
	public function get_product_options() {
		...
		if(flms_is_module_active('woocommerce')) {
			...
			//show link to the product
			$product_link_actions = '';
			$product_id = get_post_meta($post->ID, 'flms_woocommerce_product_id', true);
			if($product_id != '') {
				$product_link_actions .= '<div class="settings-field">';
					$product_link_actions .= '<label>Extra product configuration</label>';
					$product_link_actions .= '<p class="description">If you need to add extra functionality to the product, such as shipping rates, you will need to edit the product itself.';
					if(apply_filters('flms_generate_new_products_on_import', false)) {
						$product_link_actions .= ' <strong>You have set the importer to create new products on import. Any changes to this product will be lost during the next import.</strong>';
					}
					if(apply_filters('flms_generate_new_products_on_save', false)) {
						$product_link_actions .= ' <strong>You have applied a filter to generate a new product when this course is saved. Any changes to this product will be lost when this course is saved.</strong>';
					}
					$product_link_actions .= '</p>';
					$product_link_actions .= '<a href="'.get_edit_post_link($product_id).'" title="View product" target="_blank" class="button button-primary">';
						$product_link_actions .= 'Edit course product';
					$product_link_actions .= '</a>';
				$product_link_actions .= '</div>';
			}

			$product_link_actions .= '<div class="settings-field">';
				$product_link_actions .= '<label><strong>Danger Zone</strong></label>';
				$product_link_actions .= '<p class="description">If you have issues with the product associated with this course, you can generate a new one by checking the box below.</p>';						
				$product_link_actions .= '<label><input type="checkbox" name="flms-force-generate-new-product" />Generate a new product ID when this course is updated</label>';
			$product_link_actions .= '</div>';
	...
	public function get_version_options($layout) {
		...
		$return = '<div class="version-options">';
			...
			if(is_array($versions)) {
				...
				$return .= '<div class="settings-field">';
					$return .= '<div class="setting-field-label">';
						$return .= 'Version Title Display ';
						$return .= '<div class="flms-tooltip" data-tooltip="Choose the title format for this version, used when a newer course version is published"></div>';
					$return .= '</div>';
					if($layout != 'grid') {
						$return .= '<p class="description">Choose the title format for this version, used when a newer course version is published</p>';
					}
					$status_value = apply_filters('flms_default_version_title_format','default');
					if(isset($versions["{$active_version}"]['version_title_display'])) {
						$status_value = $versions["{$active_version}"]['version_title_display'];
					}
					$options = array(
						'default' => 'Course Title (Version name)',
						'hide-version-name' => 'Course Title',
						'replace-title' => 'Version name',
					);
					$return .= '<select name="version-title-display">';
						foreach($options as $k => $v) {
							$return .= '<option value="'.$k.'"';
							if($k == $status_value) {
								$return .= ' selected';
					...
					$status_value = apply_filters('flms_default_force_version_title_format_override','');
					if(isset($versions["{$active_version}"]['force_version_title_on_latest'])) {
						$status_value = $versions["{$active_version}"]['force_version_title_on_latest'];
					}
					$checked = '';
					if($status_value != '') {
						$checked = ' checked="checked"';
					}
					$return .= '<label class="setting-field-label sublabel"><input type="checkbox" name="force-version-title-for-latest" value="1" '.$checked.' /> Use this format for the most recent version of the course';
					if($flms_latest_version == $active_version) {
						$return .= ' (this version).';
					} else {
						$return .= '. <em>This is not the latest version and will use the above settings by default.</em>';
					}
					$return .= '</label>';
				$return .= '</div>';
				...
				$return .= '<div class="settings-field">';
					$return .= '<div class="setting-field-label">';
						...
						$return .= '<div class="flms-tooltip" data-tooltip="Draft course versions are not visible to enrollees. Archived courses will no longer appear for enrollment but stay in the users account."></div>';
					$return .= '</div>';
					if($layout != 'grid') {
			...
			$lesson_list .= '">';
				if($editing_post_type != $post_type) {
					$lesson_list .= '<div class="item-header">';
						...
						$lesson_list .= flms_get_the_title($post_id); // .' - '.$post_id;
						if($post_type == 'flms-lessons') {
							if($sample) {
		...
		if(isset($_POST['flms-post-type'])) {
			...
			if($_POST['flms-post-type'] == 'flms-courses') {
				...
				if(flms_is_module_active('course_tabs')) {
					...
					$tabs = $course_tabs->get_course_tab_fields(true, true, true);
					if(!empty($tabs)) {
						$course_tabs->save_course_tab_content($post_id, $active_version, $_POST);
					}
				}
				...
				if(flms_is_module_active('woocommerce')) {
					...
					$force_new_product_creation = apply_filters('flms_generate_new_products_on_save', false);
					if(isset($_POST['flms-force-generate-new-product'])) {
						$force_new_product_creation = true;
					}
					$woo->create_update_course_product($course_id, $force_new_product_creation);
				}
	...
	public function update_course_query_metadata($course_id) {
		...
		$title = strip_tags($course->get_course_version_name($flms_latest_version));
		...
		if(isset($flms_course_version_content[$flms_latest_version])) {
			...
			if(isset($latest_data['course_numbers'])) {
				foreach($latest_data['course_numbers'] as $k => $v) {
					if($v != '') {
						$key = 'course_number_'.$k;
						$values[] = array(
							$course_id,
							$key,
							$v,
			...
			if(isset($latest_data['version_status'])) {
				
				$values[] = array(
					$course_id,
					'course_status',
					$latest_data['version_status'],
				);
			...
			if(isset($latest_data['course_tabs'])) {
				if(is_array($latest_data['course_tabs'])) {
					foreach($latest_data['course_tabs'] as $key => $value) {
						if($value != '') {
							$values[] = array(
								$course_id,
								'course_tab_'.$key,
		...
		do_action('flms_after_course_metadata_update');
	}
	...
	public function update_exam_settings($post_id, $active_version, $data) {
		...
		delete_metadata(
			...
			"flms_current_exam_questions_{$post_id}_$active_version", // the meta key to be removed everywhere
	 ...
	 */
	public function update_version_attributes($post_id,$active_version, $postdata) {
		...
		if(isset($postdata['version-name'])) {
			$version_name = $postdata['version-name'];
		}
		if($version_name == '') {
			$version_name = "Version $active_version";
		}
		...
		if(isset($postdata['version-permalink'])) {
			$version_permalink = $postdata['version-permalink'];
		}
		if($version_permalink == '') {
			$version_permalink = "version-$active_version";
		}
		...
		if(isset($postdata['version-status'])) {
			$version_status =$postdata['version-status'];
		}
		$course_versioned_content["{$active_version}"]['version_status'] = sanitize_text_field($version_status);

		$version_title_format = 'default';
		if(isset($postdata['version-title-display'])) {
			$version_title_format = $postdata['version-title-display'];
		}
		$version_title_display = sanitize_text_field($version_title_format);
		$course_versioned_content["{$active_version}"]['version_title_display'] = $version_title_display;

		$force_version_display = '';
		if(isset($postdata['force-version-title-for-latest'])) {
			$force_version_display = $postdata['force-version-title-for-latest'];
		}
	...
	/**
	 * Clean up the post data on course or lesson deletion - this doesn't work, well some of it. Deleting products works
	 */
	public function trash_clean_course_postdata($post_id, $post) {
		...
		if(!in_array($post_type, flms_get_plugin_post_type_internal_permalinks())) {
			return;
		}
		...
		if ( $post_type == 'flms-lessons') {
		...
		} else if ($post_type == 'flms-courses') {
			$versioned_content = get_post_meta($post_id,'flms_version_content',true);
			if(is_array($versioned_content)) {
			...
			if($product_id != '') {
				...
				if($product !== false) {
					$product->delete(); 
					//flms_debug('Trashed post '.$product_id, 'Product deleting hook fired for '.$post_id);
				}
			}
	...
	public function trash_clean_course_query_metadata($post_id) {
		global $wpdb;
		$table = FLMS_COURSE_QUERY_TABLE;
		//remove old data
	...
	public function copy_course_content($post_id, $source_version, $active_version, $versions = null ) {
		...
		if(isset($versions["$source_version"]["course_preview"])) {
			$versions["$active_version"]["course_preview"] = $versions["$source_version"]["course_preview"];
		}
		...
		if(isset($versions["{$source_version}"]["course_certificates"])) {
			$versions["{$active_version}"]["course_certificates"] = $versions["{$source_version}"]["course_certificates"];
		}
		...
		if(flms_is_module_active('course_credits')) {
			...
			if(isset($versions["$source_version"]['course_numbers'])) {
				$versions["$active_version"]['course_numbers'] = $versions["$source_version"]['course_numbers'];
			}
	...
	public function delete_course_version($post_id, $version, $versions = null) {
		...
		if(is_array($course_exams)) {
			foreach($course_exams as $exam_id) {
				$exam_versions = get_post_meta($exam_id,'flms_version_content',true);
				if(isset($exam_versions["$version"])) {
					unset($exam_versions["$version"]);
				}
				update_post_meta($exam_id,'flms_version_content',$exam_versions);
			}
