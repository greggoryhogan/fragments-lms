...
class FLMS_Ajax {
	...
	public function __construct() {
		$this->ajax_functions = array(
			...
			'delete_course_history_entry',
			'add_course_to_group',
			'remove_course_from_group',
			'remove_user_course_from_group',
		);
	...
	public function search_questions_callback() {
		...
		if($question_query->have_posts()) {
			while ($question_query->have_posts()) {
				...
				$post_id = get_the_ID();
				$results .= '<div class="exam-question"><label><input type="checkbox" value="'.$post_id.'">'.flms_get_the_title($post_id).'</label></div>';
			}
		} else {
	...
	public function paginate_exam_callback() {
		...
		if($review == 0) {
			if($reset_exam_progress == 1) {
				...
				$course_progress->log_user_activity($user_id, $exam_id, $version_index, 'started');
			}
		...
		$exam_questions = maybe_unserialize(get_user_meta($user_id, "flms_current_exam_questions_{$exam_id}_$version_index", true)); 
		if(apply_filters('flms_debug_exam_questions', false)) {
			$exam_questions = ''; //for debugging
		}
		if($exam_questions == '') {
			$exam_questions = flms_get_exam_questions($exam_id, $version_index);
			update_user_meta($user_id, "flms_current_exam_questions_{$exam_id}_$version_index", maybe_serialize($exam_questions));
		}
	...
	public function grade_exam_callback() {
		...
		if(isset($grade['submission_error'])) {
			if($grade['submission_error'] == 1) {
				wp_send_json($grade);
				wp_die();
			}
		}
	...
	public function enroll_user_in_course_callback() {
		...
		$accepts_reporting = absint($_POST['accepts_reporting']);
		if($accepts_reporting > 0) {
			$meta_id = $response['id'];
	...
	public function search_flms_courses_callback() {
		...
		$args = apply_filters('flms_search_courses_args', $args, $term);
		$query = new WP_Query($args);
		if ($query->have_posts()) {
			while ($query->have_posts()) {
				$query->the_post();
				...
				if(is_array($course_versions)) {
					foreach($course_versions as $k => $course_version) {
						if(is_numeric($k)) {
							$version_name = strip_tags($course->get_course_version_name($k));
							$default_name = $version_name." [ID: $post_id]";
							$name = apply_filters('flms_search_courses_label', $default_name, $version_name, $post_id, $course_version );
	...
	public function get_report_type_fields_callback() {
		...
		switch($type) {
			case 'course_progress':
				...
				$args = apply_filters('flms_reports_courses_list', $args);
				...
				if(!empty($courses)) {
					...
					foreach($courses as $course) {
						$report_fields .= '<option value="'.$course->ID.'">'.get_the_title($course->ID).'</option>';
					}
			...
			case 'royalties':
				...
				break;
			default:
				$report_fields = apply_filters('flms_custom_report_fields', $report_fields, $type);
				break;
		}
		wp_send_json(
			array(
		...
		if($primary_action == 'answers') {
			...
			if(!empty($exams)) {
				...
				foreach($exams as $exam) {
					$report_fields .= '<option value="'.$exam.'">'.flms_get_the_title($exam).'</option>';
				}
				$report_fields .= '</select>';
				$report_fields .= '</div>';
			}
	...
	public function export_content_callback() {
		...
		do_action('flms_before_export_content', $type);
		switch ($type) {
			...
			case 'courses':
				...
				if($query->have_posts()) {
					while($query->have_posts()) {
						...
						$fields = '';
						$id = get_the_ID();
						$title = flms_get_the_title($id,'export_content');
								...
								if(is_numeric($version)) {
									...
									foreach($fields_to_process as $field) {
										if(isset($version_settings[$field])) {
											...
											} else if($field == 'course_certificates') {
												$certs = array();
												foreach($version_settings[$field] as $certificate_id) {
													$certs[] = get_the_title($certificate_id);
												}
												$fields[] = implode('|', $certs);
						...
						if(is_array($lesson_settings)) {
							...
							foreach($course_versions as $version => $version_settings) {
								...
								if(isset($topic_version_array[$version])) {
									...
									$lesson_name = flms_get_the_title($lesson_id);
									$lesson = new FLMS_Lesson($lesson_id);
									$flms_active_version = $version;
						...
						if(is_array($lesson_settings)) {
							...
							foreach($course_versions as $version => $version_settings) {
								if(is_numeric($version)) {
									...
									$fields = array($id,$title,$course_title,$lesson_id,$lesson_type,$version,$lesson_order);
									$fields_to_process = array(
										'post_content',
									);
									foreach($fields_to_process as $field) {
										...
										foreach($fields_to_process as $field) {
											if(isset($exam_settings[$field])) {
												if($field == 'cumulative_exam_questions') {
													...
													$fields[] = "$string";
												} else if ($field == 'exam_question_categories') {
													$strings = array();
													foreach($exam_settings[$field] as $exam_category) {
														$cat  = get_term_by('term_id', $exam_category, 'flms-question-categories');
														if($cat !==  false) {
															$strings[] = $cat->name;
														}
													}
			...
			case 'user-data':
				...
				if($total_users > 1000) {
					fclose($open);
					delete_option('flms_userdata_export');
					wp_clear_scheduled_hook('flms_export_user_data');
					//init cron event
					$data = array(
						'filename' => $filename,
						'processed_users' => 0,
					...
					wp_schedule_single_event( time(), 'flms_export_user_data', $data );
					wp_send_json(
						array(
							'alert' => 'Export started. You will be notified when your file is ready for download'
						)
					);
				} else {
					$iteration = 100;
					...
					while($processed_users <= $total_users) {
						...
						foreach($users as $user) {
							$fields = flms_export_user_data($user);
							fputcsv($open,$fields,$separator);
							$processed_users++;
						}
						$iterations++;
		...
		do_action('flms_after_export_content', $type);
	...
	public function import_content_callback() {
		...
		switch ($type) {
			case 'plugin-settings':
				...
				break;
			case 'courses':
				delete_option('flms_course_import');
				//delete transient
				//delete_transient('flms_course_import_data');
				delete_option('flms_course_import_ct');
			...
			case 'topics':
				...
				break;
			case 'exams':
				delete_option('flms_exams_import');
				delete_option('flms_exams_import_ct');
				//init cron event
				$file_url = flms_local_attachment_url($attachment_id);
				$data = array(
					'file' => $file_url,
			...
			case 'questions':
				...
				delete_option('flms_questions_import');
				//delete transient
				//delete_transient('flms_course_import_data');
				delete_option('flms_questions_import_ct');
			...
			case 'user-data':
				//init cron event
				update_option('flms_user_export', "0%");
				$file_url = flms_local_attachment_url($attachment_id);
				$data = array(
					'file' => $file_url,
					'import_action' => $import_action,
	...
	public function cancel_import_callback() {
		$import_type = sanitize_text_field($_POST['type']);
		$flms_cron_event = "flms_import_$import_type";
		//$cron_jobs = wp_get_ready_cron_jobs();

		$cron_jobs = get_option('cron'); // Fetch the entire cron schedule.

		if (!$cron_jobs || !is_array($cron_jobs)) {
		//	error_log('No scheduled cron jobs found.');
			return;
		}

		delete_option("flms_{$import_type}_import");
		delete_option("flms_{$import_type}_import_ct");
		...
		foreach ($cron_jobs as $timestamp => $hooks) {
			...
			foreach ($hooks as $hook => $events) {
				if ($hook === $flms_cron_event) {
					foreach ($events as $event_key => $event) {
						$args = isset($event['args']) ? $event['args'] : array();
						// Clear the event
						wp_unschedule_event($timestamp, $flms_cron_event, $args);
						set_transient( 'flms_import_canceled', $import_type );
					}
				}
			}
		}
		...
		switch($type) {
			case 'courses':
				...
				if(!empty($courses)) {
					...
					foreach($courses as $course) {
						$report_fields .= '<label><input type="checkbox" value="'.$course->ID.'" />'.get_the_title($course->ID).'</label>';
					}
					$report_fields .= '</div>';
					$report_fields .= '</div>';
				}
			...
			case 'lessons':
				...
				if(!empty($lessons)) {
					...
					foreach($lessons as $lesson) {
						...
						//$report_fields .= '<label><input type="checkbox" value="'.$lesson->ID.'" />'.$lesson->post_title.' ('.flms_get_the_title($flms_course_id).')</label>';
					}
					foreach($course_lessons as $k => $v) {
						$report_fields .= '<label><input type="checkbox" value="'.$k.'" />'.flms_get_the_title($k);
						foreach($v as $lesson_id) {
							$report_fields .= '<label><input type="checkbox" value="'.$lesson_id.'" />'.flms_get_the_title($lesson_id).'</label>';
						}
						$report_fields .= '</label>';
					}
					$report_fields .= '</div>';
			...
			case 'exams':
				...
				if(!empty($lessons)) {
					...
					foreach($course_lessons as $k => $v) {
						if($k > 0) {
							$report_fields .= '<label><input type="checkbox" value="'.$k.'" />'.flms_get_the_title($k, 'exporter');
							foreach($v as $lesson_id) {
								$report_fields .= '<label><input type="checkbox" value="'.$lesson_id.'" />'.flms_get_the_title($lesson_id, 'exporter').'</label>';
							}
							$report_fields .= '</label>';	
						} else {
							foreach($v as $lesson_id) {
			...
			case 'user-data':
				...
				$total_users = flms_get_site_user_count();
				if($total_users > 1000) {
					$report_fields .= '<div><strong>You have a large number of users ('.number_format($total_users).'). This process will be run in the background and you will be notified when your file is ready for download.</strong></div>';
				} else {
					$report_fields .= '<div>Export '.number_format($total_users) .' users.</div>';
				}
	...
	public function get_course_completed_users_callback() {
		...
		$sql_query = $wpdb->prepare("SELECT id, customer_id FROM $table WHERE course_id=%d AND course_version=%d ORDER BY id DESC", $course_data[0], $course_data[1]);
		...
		if(!empty($results)) {
			...
			foreach($results as $result) {
				if(!in_array($result->customer_id, $taken_users)) {
					...
					$user_list .= '<option value="'.$user->ID.'" data-entry-id="'.$result->id.'">'.$user->display_name.'</option>';
				}
			}
			$user_list .= '</select>';
            ...
            if(!empty($open_seats)) {
				...
				foreach($open_seats as $course => $data) {
					...
					foreach($data as $order_info) {
						//$return .='<div>'.flms_get_the_title($course_id).'</div>';
						//available seats
	...
	public function leave_group_callback() {
		...
		if($is_admin) {
			...
			wp_send_json(
				array(
					'success' => 1,
					'new_html' => $groups->get_group_member_admin_content($post_id),
					'new_course_html' => $groups->get_group_courses_content($post_id)
				)
			);
		} else {
			if($removed !== false) {
	...
	public function manager_invitation_callback() {
		...
		$name = flms_get_the_title($group_id);
		if(!empty($managers)) {
	...
	public function user_group_enroll_callback() {
		...
		if(isset($_GET['user_id'])) {
			...
			if(isset($_GET['user_email'])) {
				$is_admin_request = true; //coming from groups management page in admin
				$user_emails = explode(',',$_GET['user_email']);
				foreach($user_emails as $email) {
					if($email != '') {
						$user_email = sanitize_email(trim($email));	
						$user = get_user_by('email',$user_email);
						if($user === false) {
							if(apply_filters('flms_group_enrollment_autocreate_notfound_user', false)) {
								$user_pass = wp_generate_password();
								/*if(function_exists('wc_create_new_customer')) {
									$user_id = wc_create_new_customer($user_email, $user_email, wp_generate_password());
								} else {
								...
								if(!is_wp_error( $user_id )) {
									...
									if(apply_filters('flms_group_enrollment_autocreate_notfound_user_notification_email', true)) { // && !function_exists('wc_create_new_customer')
										if( class_exists( 'WC_Emails' ) ) {
											$WC_Emails = WC_Emails::instance();
											//$WC_Emails->customer_new_account( $user_id );
											$WC_Emails->customer_new_account( $user_id, array('user_pass' => $user_pass), true );
										} else {
											$notify = apply_filters('flms_group_enrollment_autocreate_notfound_user_notification_email_to', 'user');
											wp_new_user_notification( $user_id, null, $notify );
										}
									}
								} else {
									$errors[] = "Could not create user with email $user_email.";
								}
								
							} else {
								$errors[] = "No user found with email $user_email.";
							}
							/** TODO: AUTOMATICALLY CREATE USER BY EMAIL? */
						} else {
							$user_ids[] = $user->ID;
						}
					}
		...
		if(!empty($user_ids)) {
			foreach($user_ids as $user_id) {
				//see if we need to autoenroll them in the group
				if($is_admin_request) {
					if(apply_filters('flms_autoenroll_group_user_during_enrollment', true)) {
						$group_members = get_post_meta($group_id, 'flms_group_member');
						...
						foreach($group_courses as $course_info => $course_settings) {
							if($row == $index) {
								...
								if(isset($course_data[0]) && isset($course_data[0])) {
									...
									if($seats == 0 && $is_admin_request && apply_filters('flms_disregard_admin_group_enrollment_seats_max', true)) {
										$seats = 1;
									}
									if($seats > 0) {
										$course_id = $course_data[0];
										...
										$errors[] = "Not enough seats to enroll $user_email.";
									}
								}
							}
							$row++;
		...
		if($is_admin_request) {
			$new_html = $groups->get_group_courses_content($group_id);
			$new_member_html = $groups->get_group_member_admin_content($group_id);
		...
		wp_send_json(
			array(
				'success' => $success,
				'new_html' => $new_html,
				'new_member_html' => $new_member_html,
				'errors' => implode('<br>',$errors),
			)
		);
	}

	public function update_completion_date_callback() {
		global $wpdb;
		...
		if($updated !== false) {
			...
			wp_send_json(
				array(
					'success' => 0,
					'response' => 'Error updating completion date.',
				)
			);
		}
	}

	public function delete_course_history_entry_callback() {
		global $wpdb;
		$entry_id = absint($_POST['entry_id']);
		$table = FLMS_ACTIVITY_TABLE;
	...
	public function add_course_to_group_callback() {
		$group_id = absint($_GET['group_id']);
		$course_id = absint($_GET['course_id']);
		$paid_fees = absint($_GET['paid_fees']);
		$reporting_fees = array();
		$course_version = absint($_GET['course_version']);
		...
		if(!isset($group_courses[$course_identifier])) {
			$group_courses[$course_identifier] = array();
		}
		if(isset($group_courses[$course_identifier]['reporting_fees'])) {
			$reporting_fees = $group_courses[$course_identifier]['reporting_fees'];
		} else {
			$reporting_fees = array();
		}
		$seats = (int) sanitize_text_field($_GET['seats']);
		...
		if(flms_is_module_active('course_credits')) {
			...
			foreach($credit_fields as $credit_type) {
				...
				if($status != 'none') {
					...
					if(is_array($reporting_fees[$meta_key])) {
						$accepted = $reporting_fees[$meta_key]['accepted'];
						$declined = $reporting_fees[$meta_key]['declined'];
					} else {
						$accepted = 0;
						$declined = 0;
						$reporting_fees[$meta_key] = array('accepted' => 0, 'declined' => 0);
					}
					if($paid_fees > 0) {
						$accepted += $seats;
					} else {
					...
					$reporting_fees[$meta_key] = array('accepted' => $accepted, 'declined' => $declined);	
					
				}
			}
		}
	
		if(isset($group_courses[$course_identifier]['seats'])) {
			$seats += $group_courses[$course_identifier]['seats'];
		}
		$enrolled = array();
		if(isset($group_courses[$course_identifier]['enrolled'])) {
			$enrolled = $group_courses[$course_identifier]['enrolled'];
		}
		$group_courses[$course_identifier] = array(
			'seats' => $seats,
			'reporting_fees' => $reporting_fees,
			'enrolled' => $enrolled
		);
		update_post_meta($group_id, 'group_courses',$group_courses);
	...
	public function remove_course_from_group_callback() {
		$group_id = absint($_GET['group_id']);
		$course_index = absint($_GET['course_index']);
		$groups = new FLMS_Module_Groups();
		$group_courses = get_post_meta($group_id, 'group_courses',true);
		if(is_array($group_courses)) {
			$counter = 0;
			foreach($group_courses as $index => $value) {
				if($counter == $course_index) {
					//get users
					if(isset($value['enrolled'])) {
						if(is_array($value['enrolled'])) {
							$course_data = explode(':',$index);
							$course_id = $course_data[0];
							$course_version = $course_data[1];
							$user_progress = new FLMS_Course_Progress();
							if(isset($course_data[0]) && isset($course_data[0])) {
								foreach($value['enrolled'] as $user_id) {
									$user_progress->unenroll_user($user_id, $course_id, $course_version);
								}
							}
						}
	...
	public function remove_user_course_from_group_callback() {
		$user_id = absint($_GET['user_id']);
		$group_id = absint($_GET['group_id']);
		$course_index = absint($_GET['course_index']);
		$groups = new FLMS_Module_Groups();
				...
				if($counter == $course_index) {
					...
					if(isset($value['enrolled'])) {
						if(is_array($value['enrolled'])) {
							...
							if(($key = array_search($user_id, $value['enrolled'])) !== false) {
								unset($value['enrolled'][$key]);
							}
							$group_courses[$index]['enrolled'] = $value['enrolled'];
						}
					}
					update_post_meta($group_id, 'group_courses',$group_courses);
					break;
