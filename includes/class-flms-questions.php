...
class FLMS_Questions {
	...
	public function question_bank_query_output($current_questions, $paged) {
		...
		if($questions_query->have_posts()) {
			while($questions_query->have_posts()) {
				...
				$questions .= ' />';
				$questions .= mb_strimwidth(flms_get_the_title(), 0, 100, "...");;
	...
	public function get_question_taxonies($checkbox = false, $checked_array = array(), $parent = false, $show_link = false, $show_number_input = false, $numbers_array = array()) {
		...
		if(!empty($terms)) {
			...
			foreach($terms as $term) {
				...
				if($show_link) {
					$return .= '<a href="'.admin_url('edit.php?flms-question-categories='.$term->slug).'&post_type=flms-questions&orderby=menu_order&order=asc" target="_blank">View all questions</a>';;
				}
				$return .= '</li>';
			}
			$return .= '</ul>';
	...
	public function flms_output_exam_questions($post_id, $exam_questions, $user_id, $exam_identifier, $limit = 10, $starting_index = 0, $start_count = 0, $page = 1, $review = 0, $exam_is_graded = 'graded', $display_type = 'exam') {
		...
		if($display_type != 'print') {
			...
			if(apply_filters('flms_show_exam_pagination', true)) {
				$pages = ceil($exam_count / $limit);
				if($pages > 1) {
					$questions .= '<div class="flms-flex mobile-flex align-center flms-pagination">';
					for ($x = 1; $x <= $pages; $x++) {
						$questions .= '<button class="button button-primary exam-pagination-navigation';
						if($x == $page) {
							$questions .= ' current-item';
						}
						$questions .= '" data-page="'.$x.'">'.$x.'</button>';
					}					  
					$questions .= '</div>';
				}
			}
			...
			$questions .= '<div class="flms-navigation exam-navigation">';
				...
				if($review == 0) {
					...
					if($save_enabled == 'active' || $save_enabled == 1) {
