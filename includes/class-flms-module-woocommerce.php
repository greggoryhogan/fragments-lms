...
class FLMS_Module_Woocommerce {
    ...
    public function flms_init_woo_actions_and_filters() {
        ...
        if(flms_is_module_active('course_credits')) {
            ...
            add_filter('woocommerce_order_actions', array($this,'my_custom_woocommerce_order_actions'), 10, 2);
            add_action('woocommerce_process_shop_order_meta', array($this,'my_custom_woocommerce_order_action_execute'), 50, 2);
            ...
            //add group seats to order totals
            add_filter('woocommerce_checkout_cart_item_quantity', array($this, 'flms_cart_item_quantity_group_seats'),10,3);
            add_filter('woocommerce_order_item_name', array($this, 'flms_add_course_number_to_item_name'), 10, 3);
            add_filter('woocommerce_order_item_quantity_html', array($this, 'flms_order_item_quantity_group_seats'),10,2);
            
        }

        if(flms_is_module_active('groups')) {
            add_action( 'woocommerce_before_calculate_totals', array($this, 'group_purchase_price_adjustments'), PHP_INT_MAX, 1);
        }
    ...
    public function flms_variation_has_course_notice() {
        global $post;
        if(isset($post->ID)) {
            $course_id = get_post_meta($post->ID, 'flms_woocommerce_product_id', true);
            if($course_id != '') {
                echo '<div class="product-has-course-notice toolbar">';
                echo '<p><em>This product is associated with a '.FLMS_PLUGIN_NAME.' course. Any changes in prices should be defined at the course level. Any changes at the product level will be overwritten when the course is updated. <a href="'.get_edit_post_link($course_id).'">Edit course</a></em></p>';
                echo '</div>';
            }
        }
    }
    ...
    public function group_purchase_price_adjustments($cart) {
        ...
        foreach ( $cart->get_cart() as $cart_item ) {
            ...
            if(isset($flms_settings['woocommerce']['groups_bulk_purchase_discount_status'])) {
                $discount_status = $flms_settings['woocommerce']['groups_bulk_purchase_discount_status'];
        ...
        remove_action( 'woocommerce_before_calculate_totals', array($this, 'group_purchase_price_adjustments'), PHP_INT_MAX, 1);
        return $cart;
    }
    ...
    public function get_woocommerce_module_fields() {
        ...
        if(flms_is_module_active('course_taxonomies') || flms_is_module_active('course_materials')) {
            $section_settings_heading = array(
                array(
                    'label' => '',
                    'key' => 'woo-specific-settings',
                    'type' => 'section_heading',
                    'description' => 'The settings below effect the display when you manually create a product, not for the default course display. To change the default settings, please go to the "Design and Layout" tab.',
                    'default' => '75'
                ),
            );
            $fields = array_merge($fields, $section_settings_heading);
        }
            ...
            $credits = '<table class="shop_table shop_table_responsive">';
                $credits .= '<tbody>';
                    foreach($cart_credits as $k => $v) {
                        ...
                        $credits .= '<tr><th>'.$label.'</th><td data-title="'. esc_attr($label).'">'.$v.'</td></tr>';
                    }
                $credits .= '</tbody>';
            $credits .= '</table>';
        } else {
    ...
    public function my_courses_tab_content() {
        ...
        echo apply_filters('flms_active_courses_heading', "<h2>Active $course_name</h2>");
        ...
        if(!is_array($completed_courses)) {
            ...
            echo apply_filters('flms_completed_courses_heading', '<h2 class="mt-4">Completed '.$course_name.'</h2>');
    ...
    public function my_credits_tab_content() {
        ...
        $course_credits_heading = apply_filters('flms_course_credits_in_process_heading', "$course_name in Process");
        echo "<h2>$course_credits_heading</h2>";
        echo $notice;

        $active_courses = flms_get_user_completed_courses();
        if(!is_array($active_courses)) {
            $notice = '<p>You have not earned any '.strtolower($course_name).'.</p>';
        } else if(empty($active_courses)) {
            $notice = '<p>You have not earned any '.strtolower($course_name).'.</p>';
        } else {
            $credits = array();
            ...
            $notice = '<div class="credit-summary flms-table mobile-collapse flms-background-border">';
                $notice .= '<div class="credit-item flms-table-item heading flms-background-border">';
                    $notice .= '<div class="type heading col-1-3 flms-background-border">';
                        $notice .= 'Completed';
                    $notice .= '</div>';
                    $notice .= '<div class="type heading col-1-3 flms-background-border">';
                        $notice .= $course_label;
                ...
                $notice .= '</div>';
                foreach($active_courses as $active_course) {
                    $course = new FLMS_Course($active_course['course_id']);
                    $credits = $course_credits->get_course_credits($active_course, $credits);
                    $has_credits = false;
                    $course_credits_data = array();
                    foreach($credits as $k => $v) {
                        $user_has_license = get_user_meta( $user_id, "flms_has-license-$k", true);
                        $user_license_number = get_user_meta( $user_id, "flms_license-$k", true);
                        $required = 'none';
                        if(isset($flms_settings['course_credits'][$k]["license-required"])) {
                            $required = $flms_settings['course_credits'][$k]["license-required"];
                        }
                        $continue = true;
                        ...
                        if($continue) {
                            $has_credits = true;
                            $course_credits_data[$k] = $v;
                        }
                    }
                    if($has_credits) {
                        $notice .= '<div class="credit-item flms-table-item flms-background-border">';
                            $notice .= '<div class="type col-1-3 flms-background-border" data-label="Completed">';
                                $notice .= date($date_format, strtotime($active_course['completion_date']));
                            $notice .= '</div>';
                            $notice .= '<div class="type col-1-3 flms-background-border" data-label="'.$course_label.'">';
                                $notice .= $course->get_course_version_name($active_course['course_version']);
                            $notice .= '</div>';
                            if(!empty($course_credits_data)) {
                                $credits_count = count($course_credits_data);
                                $notice .= '<div class="type col-1-3 flms-background-border" data-label="'.$course_name.'">';
                                    $i = 0;
                                    foreach($course_credits_data as $k => $v) {
                                        $notice .= flms_get_label($k).': '.$v;
                                        if(++$i != $credits_count) {
                                            $notice .= '<br>';
                                        }
                                    }
    ...
    public function purchase_course_options($post_id,$course_version) {
        ...
        $no_products_text = '<p class="no-purchase-option">There are no options to purchase this course.</p>';
        if(current_user_can('manage_woocommerce')) {
            $no_products_text .= ' <a href="'.admin_url('edit.php?post_type=product').'" class="btn">Assign course to product</a> <a href="'.admin_url('post.php?post='.$post_id.'&action=edit').'" class="btn">Update course access type</a>';
        } else if(current_user_can('edit_others_posts')) {
            $no_products_text .= ' <a href="'.admin_url('post.php?post='.$post_id.'&action=edit').'" class="btn">Update course access type</a>';
        } else {
            //$return .= 'There are no purchase options for this course currently.';
        }
        $no_products_text = apply_filters('flms_no_purchase_option', $return, array($post_id,$course_version));
        
        if(empty($products)) {
            return $no_products_text;    
        }
        ...
        $product_options = apply_filters('flms_course_product_options', $product_options, $post_id, $course_version);
        if(!empty($product_options)) {
            if(count($product_options) > 1) {
                $return = '<p>There are multiple purchase options for this course:</p>';
                $return .= '<ul>';
                foreach($product_options as $product_option) {
                    $return .= '<li><a href="'.get_permalink($product_option).'">'.flms_get_the_title($product_option).'</a></li>';
                }
                $return .= '</ul>';
            } else {
    ...
    function enroll_on_status_change( $order_id, $old_status, $new_status, $order ) {
        if($new_status == 'completed') {
            ...
            $this->remove_course_access($order_id);
        }
    }
    ...
    public function enroll_customer_courses($order_id) {
        ...
        if ( $order ) {
            $previously_processed = get_post_meta($order_id, 'flms_customer_order_enrollment_processed', true);
            if($previously_processed != '') {
                //$order->add_order_note(__('Customer course(s) enrollment previously processed. Skipping...', 'flms'));
                return;
            }
                ...
                if(is_array($courses)) {
                    if(!empty($courses)) {
                        ...
                        foreach($courses as $course) {
                            ...
                            $course_progress = new FLMS_Course_Progress();
                            if(apply_filters('flms_log_user_purchases_in_activity_table', false)) {
							    $course_progress->log_user_activity($customer_id, $course_id, $version, "purchased");
                            }
            ...
            //flag so this doesn't happen multiple times
            update_post_meta($order_id, 'flms_customer_order_enrollment_processed', 1);
            //log it in the order
            $order->add_order_note(__('Customer enrolled in purchased course(s)', 'flms'));
        }
    }
    ...
    public function remove_course_access($order_id, $products = array()) {
        ...
        if ( $order ) {
            ...
            delete_post_meta($order_id, 'flms_customer_order_enrollment_processed');
        }
    }
    ...
    public function display_group_purchase() {
        ...
        $html .= '<table class="variations-style"><tbody><tr><th class="label"><label>'.$label.' <span class="flms-tooltip" data-tooltip="Purchase courses on behalf of other users and allow them to enroll for free through your group."></span></label></th><td class="value">';
            $show_group_purchase_options = apply_filters('flms_group_purchase_option_for_empty_user_groups', true);
            if(!$show_group_purchase_options) {
                $groups = new FLMS_Module_Groups();
                $current_user = wp_get_current_user();
                if($current_user->ID > 0) {
                    $user_group_ids = $groups->get_user_group_ids($current_user);
                    if(!empty($user_group_ids)) {
                        $show_group_purchase_options = true;
                    } else {
                        $show_group_purchase_options = false;
                    }
                }
            }
            if($show_group_purchase_options) {
                $html .= '<select name="group_purchase_enabled" id="group_purchase_enabled">';
                    $options = array(
                        'individual' => apply_filters('flms_individual_purchase_option_text', 'Individual'),
                        'group' => apply_filters('flms_group_purchase_option_text', flms_get_label('groups_singular')),
                    );
                    $count = 0;
                ...
                $html .= apply_filters('flms_no_group_purchase_option_text', '<span>To enable group purchases, first create a group.</span>');
            }
        $html .= '</td></tr></tbody></table>';
    ...
    function vp_cart_display_commodity_code( $item_name, $cart_item, $cart_item_key ) {
        ...
        if(flms_is_module_active('course_numbers')) {
            //$item_name .= '<pre>'.print_r($cart_item, true).'</pre>';
            $courses = '';
                ...
                foreach($courses as $course_data) {
                    ...
                    if($course_number != '') {
                        ...
                        $course_number_text = '';
                            ...
                            $course_number_text .= $course_number;
                            $item_name = "$course_number_text $item_name";
                        //$item_name .= '</div>';
                    }
    ...
    function vp_order_item_display_commodity_code( $item_id, $item, $order, $plain_text ) {
        ...
        if( $value = $item->get_meta('group_seats') ) {
            $label = flms_get_label('groups_singular') .' '.flms_get_label('seats_plural');
    ...
    public function flms_reporting_fee_product_addon_validation( $passed, $product_id, $qty ){
        ...
        $allowed = true;
        //only allow purchasing if they have completed the course
        if(!current_user_can('administrator')) {
            if(isset($_POST['group_purchase_enabled'])) {
                $purchase_type = absint($_POST['group_purchase_enabled']);
                if($purchase_type == 0) {
                    global $flms_course_id, $flms_active_version, $current_user;
                    $course_id = get_post_meta($product_id, 'flms_woocommerce_product_id', true);
                    $course = new FLMS_Course($course_id);
                    $has_access = flms_user_has_access($flms_course_id, $flms_active_version);
                    if($has_access) {
                        ...
                        if(!$completed) {
                            ...
                            wc_add_notice( "You must complete this $course_label before purchasing again", 'error' );
                            $passed = false;
                        }
                    }
                }
    ...
    public function add_reporting_fees_to_cart($cart) {
        global $flms_settings;
        $group_report_fees = apply_filters('flms_group_reporting_fees', false);
        $grouped_report_fees = array();
        $reporting_label = flms_get_label('reporting_fee');
                                ...
                                if($flms_settings['course_credits'][$credit_type]["reporting-fee-status"] != 'none') {
                                    if(isset($course_credits[$credit_type])) {
                                        ...
                                        if((int) $reporting_fee > 0) {
                                            ...
                                            if($group_report_fees) {
                                                if(!isset($grouped_report_fees[$credit_type])) {
                                                    $grouped_report_fees[$credit_type] = array(
                                                        'courses' => array(),
                                                        'total' => 0
                                                    );
                                                }
                                                $grouped_report_fees[$credit_type]['courses'][] = $product_name;
                                                $grouped_report_fees[$credit_type]['total'] += $reporting_fee;
                                            } else {
        ...
        if(!empty($grouped_report_fees)) {
            foreach($grouped_report_fees as $credit_type => $report_fee) {
                $label = flms_get_label($credit_type);
                $reporting_fee = number_format(floatval($report_fee['total']), 2);
                $name      = "$label $reporting_label";
                $name .= ' ('.implode(', ',$report_fee['courses']).')';
                $taxable   = true;
                $tax_class = '';
                $cart->add_fee( $name, $reporting_fee, $taxable, $tax_class );
            }
    ...
    public function my_custom_woocommerce_order_actions($actions, $order) {
        $previously_processed = get_post_meta($order->get_id(), 'flms_customer_order_enrollment_processed', true);
        $status = $order->get_status();
        if($status == 'completed' || $previously_processed != '') {
            return $actions;
        }
        $actions['enroll_user_via_order_action'] = __('Enroll customer courses from order', 'flms');
        return $actions;
    }

    public function my_custom_woocommerce_order_action_execute($order_id, $order) {
        if (filter_input(INPUT_POST, 'wc_order_action') !== 'enroll_user_via_order_action') {
            return;
        }
        $previously_processed = get_post_meta($order_id, 'flms_customer_order_enrollment_processed', true);
        $this->enroll_customer_courses($order_id);
        //update_post_meta($order_id, 'flms_customer_order_enrollment_processed', 1);
        $order->add_order_note(__('Customer enrolled via order action', 'enroll_user_via_order_action'));
    }
    ...
    public function woocommerce_accept_decline_flms_reporting_fee_admin(  $and_taxes, $order ) {
        if ( did_action( 'flms_woo_calculate_fees_admin' ) >= 1 )
            return;
        
        if($order->get_status() != 'pending' && $order->get_status() != 'auto-draft') {
            return;
        }
        //flms_debug($order->get_status());
	
        do_action('flms_woo_calculate_fees_admin');
        if ( is_admin()  ) { //&& ! defined( 'DOING_AJAX' )
            $existing_fees = array();
            ...
            $all_fees = array();
            foreach( $order->get_items( 'fee' ) as $item_id => $item ) {
                if(strpos($item->get_name(),'Reporting Fee') !== false) {
                    ...
                    foreach($reporting_fees as $k => $v) {
                        ...
                        if($accepted == 'Accepted') {
                            ...
                            if($courses != '') {
                                ...
                                foreach($flms_course_credit_types as $credit_type) {
                                    global $flms_settings;
                                    if($flms_settings['course_credits'][$k]["reporting-fee-status"] != 'none') {
                                        if(isset($course_credits[$k])) {
                                            $credit_amount = $course_credits[$k];
                                            $fee_type = $flms_settings['course_credits'][$k]["reporting-fee-status"];
                                            $fee = $flms_settings['course_credits'][$k]["reporting-fee"];
                                            if($fee_type == 'per-credit') {
                                                //get number of credits
                                                $reporting_fee = $credit_amount * $fee;
                                            } else {
                                            ...
                                            if(!in_array(esc_html($fee_name), $all_fees)) {
                                                $all_fees[] = esc_html($fee_name);
            ...
            if(!empty($grouped_report_fees)) {
                foreach($grouped_report_fees as $credit_type => $report_fee) {
                    ...
                    $item_fee = new WC_Order_Item_Fee();
                    $item_fee->set_name( $name ); // Generic fee name
                    $item_fee->set_amount( $reporting_fee ); // Fee amount
                    $item_fee->set_tax_class( '' ); // default for ''
                    $item_fee->set_tax_status( 'taxable' ); // or 'none'
                    $item_fee->set_total( $reporting_fee ); // Fee amount
                    $order->add_item( $item_fee );  
    ...
    public function create_update_course_product($course_id, $force_create_new_product = false) {
        
        $course = new FLMS_Course($course_id);
        global $flms_course_version_content, $flms_latest_version;

        $published_date = false;
        $course_post_data = get_post($course_id);
        if($course_post_data) {
            // Get the published date
            $published_date =  $course_post_data->post_date;
        }
        ...
        if($product !== false) {
            ...
            if($current_type == 'simple') {
                ...
                delete_post_meta($product_id,'flms_woocommerce_course_id');
                foreach($courses as $new_course) {
                    add_post_meta($product_id,'flms_woocommerce_course_id', $new_course);
                } 
                update_post_meta( $product_id, 'flms_woocommerce_simple_course_ids', $courses );
        ...
        /*if($published_date !==  false) {
            //update product date with course publish date
    ...
    public function get_products_related_to_courses( $query ) {
        ...
        if(isset($_GET['product_shipping_class'])) {
            return;
        }
    ...
    public function flms_cart_item_quantity_group_seats($text, $cart_item, $cart_item_key) {
        if ( isset( $cart_item['group_seats'] ) ){
            $value = absint($cart_item['group_seats']);
            $label = flms_get_label('groups_singular') .' '.flms_get_label('seats_plural');
    ...
    public function flms_add_course_number_to_item_name($name,$item, $is_visible) {
        if(apply_filters('flms_hide_course_numbers_in_line_items', false)) {
            return $name;
        }
        $text = '';
        if(flms_is_module_active('course_numbers')) {
    ...
    function exclude_archived_course_from_shortcode($query_args, $atts, $type) {
        //echo $type;
        // Check if this is the 'products' shortcode
        //if ($type === 'products' || $type == 'best_selling_products') {
            $results = get_transient('flms_woocommerce_published_products');
            if($results === false) {
                global $wpdb;
                ...
                $results = apply_filters('flms_woo_shortcode_product_ids', $results);
                set_transient( 'flms_woocommerce_published_products', $results );
            } 
            
            //$query_args['post__in'] = $results;
            $meta_query = [
                'relation' => 'AND', // Adjust relation if needed (e.g., 'AND')
                [
                    'key'     => 'flms_woocommerce_product_id',
                    'value'   => $results, // The value of the meta to exclude
                    'compare' => 'IN'
                ]
            ];

            $meta_query = apply_filters('flms_woo_products_shortcode_query', $meta_query);
    
            // Combine with existing meta query if necessary
