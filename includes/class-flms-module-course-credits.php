...
class FLMS_Module_Course_Credits {
    ...
    public function course_credit_summary($credits) {
        global $flms_settings;
        $credits_heading = 'Course Credits';
        if(isset($flms_settings['labels']["credits"])) {
            $credits_heading = $flms_settings['labels']["credits"];
        }
        $credit_summary = '<div class="credit-summary flms-table mobile-collapse flms-background-border">';
        $credit_summary .= '<div class="credit-item flms-table-item heading flms-background-border">';
        $type = flms_get_label('license_singular');
        $credit_summary .= '<div class="type col-1-3 flms-background-border">'.$type.'</div><div class="value col-2-3">'.$credits_heading.'</div>';
        $credit_summary .= '</div>';
        foreach($credits as $k => $v) {
            $credit_summary .= '<div class="credit-item flms-table-item mobile-2-col flms-background-border">';
                $credit_summary .= '<div class="type col-1-3 flms-background-border" data-label="'.$type.'">'.flms_get_label($k).'</div><div class="value 2-3" data-label="'.$credits_heading.'">'.$v.'</div>';
            $credit_summary .= '</div>';
        }
        $credit_summary .= '</div>';
        ...
        foreach($course_credit_fields as $field) {
            ...
            if($display != 'none' && $parent == 'none') {
                ...
                if(apply_filters('flms_show_'.$field.'_license_input', true)) {
                    $field_data .=  apply_filters('flms_woocommerce_license_field', '<input type="text" class="needs-checkbox-checked regular-text woocommerce-Input woocommerce-Input--text input-text" name="'.$field.'" id="'.$field.'" value="'.$value.'" placeholder="'.$license_label.'" '.$input_status.' />', $field, $value, $input_status);
                    if($display == 'required') {
