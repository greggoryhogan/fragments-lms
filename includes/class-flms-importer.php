...
class FLMS_Importer {
    ...
    public function flms_import_page() {
        ?><div class="wrap">
            ...
            //Show any current imports running
            $this->display_current_import_statuses();
                                ...
                                <div class="settings-field">
                                    ...
                                    <div class="flms-field select">
                                        <select name="import-type" id="import-type" class="select-toggle">
                                            ...
                                            <?php $this->get_available_import_options(); ?>
    ...
    public function get_available_import_options() {
        $course_disable = '';
        $course_import_process = get_option('flms_courses_import');
        if($course_import_process !== false) {
            $course_disable = ' disabled="disabled"';
        }
        echo '<option value="courses"  data-label="Course" data-select-toggle="#content-import-file, #importer-nav, #import-column-map-container, #import-notice" '.$course_disable.'>Courses</option>';
        echo '<option value="lessons"  data-label="Lesson" data-select-toggle="#content-import-file, #importer-nav, #import-column-map-container, #import-notice">Lessons</option>';
        echo '<option value="topics"  data-label="Topic" data-select-toggle="#content-import-file, #importer-nav, #import-column-map-container, #import-notice">Topics</option>';
        $questions_disable = '';
        $question_import_process = get_option('flms_questions_import');
        if($question_import_process !== false) {
            $questions_disable = ' disabled="disabled"';
        }
        echo '<option value="questions" data-label="Question" data-select-toggle="#content-import-file, #importer-nav, #import-column-map-container" data-toggle-hide="#import-submit-container, #import-notice" '.$questions_disable.'>Questions</option>';
        $exam_disable = '';
        $exam_import_process = get_option('flms_exams_import');
        if($exam_import_process !== false) {
            $exam_disable = ' disabled="disabled"';
        }
        echo '<option value="exams"  data-label="Exam" data-select-toggle="#content-import-file, #importer-nav, #import-column-map-container, #import-notice" '.$exam_disable.'>Exams</option>';
        echo '<option value="user-data"  data-label="User" data-select-toggle="#content-import-file, #importer-nav, #import-column-map-container, #import-notice">User Data</option>';
        //<option value="course-data" data-select-toggle="#content-import-file, #importer-nav">Course Data (Feature to come)</option>
        echo '<option value="plugin-settings" data-toggle-hide="#import-column-map-container" data-select-toggle="#settings-import-file, #importer-settings, #import-submit-container">Plugin Settings</option>';
    }

    public function display_current_import_statuses() {
        $import_caneled = get_transient( 'flms_import_canceled' );
        if ( $import_caneled != '' ) {
            ?>
            <div class="notice notice-success is-dismissible">
                <p><?php _e( FLMS_PLUGIN_NAME .' '.$import_caneled.' import canceled.', 'flms' ); ?></p>
            </div>
            <?php
            delete_transient( 'flms_import_canceled' );
        }
        $course_import_process = get_option('flms_courses_import');
        $course_import_count = get_option( 'flms_courses_import_ct' );
        if($course_import_process !== false) {
            $course_import_process = maybe_unserialize( $course_import_process );
            if(isset($course_import_process['processed'])) {
                $notice = 'A course import is currently in process.';
                $processed = absint($course_import_process['processed']);
                $total = absint($course_import_process['total']);
                if($total > 0 && $processed > 0) {
                    $notice = 'A course import is in progress. '.number_format(floatval($processed), 0) .' of '.number_format(floatval($total),0).' courses processed, '.sprintf("%.2f%%", ($processed / $total) * 100) .' complete.';
                }
                //wp_admin_notice(__( 'A course import is currently in process.', 'flms' ));
                ?>
                <div class="notice notice-success is-dismissible">
                    <p><?php echo $notice; ?> <a href="#" class="flms-cancel-import" data-flms-cancel-import="courses">Cancel import</a></p>
                </div><?php 
            }
        }
        $questions_import_process = get_option('flms_questions_import');
        if($questions_import_process !== false) {
            $questions_import_process = maybe_unserialize( $questions_import_process );
            if(isset($questions_import_process['processed'])) {
                $notice = 'A question import is currently in process.';
                $processed = absint($questions_import_process['processed']);
                $total = absint($questions_import_process['total']);
                if($total > 0 && $processed > 0) {
                    $notice = 'A question import is in progress. '.number_format(floatval($processed), 0) .' of '.number_format(floatval($total),0).' questions processed, '.sprintf("%.2f%%", ($processed / $total) * 100) .' complete.';
                }
                //wp_admin_notice(__( 'A course import is currently in process.', 'flms' ));
                ?>
                <div class="notice notice-success is-dismissible">
                    <p><?php echo $notice; ?> <a href="#" class="flms-cancel-import" data-flms-cancel-import="questions">Cancel import</a></p>
                </div><?php 
            }
        }
        $exam_import_process = get_option('flms_exams_import');
        if($exam_import_process !== false) {
            $exam_import_process = maybe_unserialize( $exam_import_process );
            if(isset($exam_import_process['processed'])) {
                $notice = 'An exam import is currently in process.';
                $processed = absint($exam_import_process['processed']);
                $total = absint($exam_import_process['total']);
                if($total > 0 && $processed > 0) {
                    $notice = 'An exam import is in progress. '.number_format(floatval($processed), 0) .' of '.number_format(floatval($total),0).' exams processed, '.sprintf("%.2f%%", ($processed / $total) * 100) .' complete.';
                }
                //wp_admin_notice(__( 'A course import is currently in process.', 'flms' ));
                ?>
                <div class="notice notice-success is-dismissible">
                    <p><?php echo $notice; ?> <a href="#" class="flms-cancel-import" data-flms-cancel-import="exams">Cancel import</a></p>
                </div><?php 
            }
        }
    }
