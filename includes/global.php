...
/**
 * Allow get_the_title to be filtered throughout the plugin
 * $post Post ID or WP_Post object
 */
function flms_get_the_title($post_object = 0, $location = 'global') {
	return apply_filters('flms_post_title', get_the_title($post_object), $post_object, $location);
}

function flms_get_post_type_label($post_type, $plural = false) {
	global $flms_settings;
	if($post_type == 'flms-questions') {
		if($plural) {
			return 'Questions';
		} else {
			return 'Question';
	...
	if(isset($course_data["course_lessons"])) {
		...
		if(!empty($lessons)) {
			...
			$lesson_html = '<div class="flms-course-summary">';
				$lesson_html .= apply_filters('flms_section_title', "<h2>$course_name $lessons_name</h2>", 'flms-lessons');
			...
			if($topic_id > 0) {
				...
				if(!empty($topic_exams)) {
					...
					foreach($topic_exams as $exam_id) {
						if($exam_id > 0) {
							$lesson_html .= '<li><a href="'.get_permalink($exam_id).'" class="flms-primary">'.flms_get_the_title($exam_id).'</a></li>';
						}
					}
					$lesson_html .= '</ul>';
				}*/
...
function flms_step_complete_checkbox($lesson_id, $completed = false) {
	...
	$checkbox = '<div class="step-checkbox '.$bg.' step-is-'.$complete.'">';
...
function flms_get_associated_exams($course_data, $wrap = false) {
	...
	return apply_filters('flms_exams_output', $lesson_html, $flms_course_id, $flms_active_version);
}
...
function flms_get_title() {
	global $post;
	$title = flms_get_the_title($post->ID);
...
if ( ! function_exists( 'flms_page_title' ) ) {
	...
	function flms_page_title( $echo = true ) {
		...
		$page_title   = flms_get_the_title( $post->ID );
...
function flms_user_has_access($post_id, $course_version = 1, $hard_query = false) {
	global $flms_user_has_access, $flms_active_version, $flms_latest_version;
	if($course_version == 1) {
...
function flms_enroll_course_button($post_id, $course_version, $version_index) {
	if(apply_filters('flms_disable_course_purchase', false, $post_id, $course_version)) {
		$no_products_text = '<p class="no-purchase-option">There are no options to purchase this course.</p>';
		return apply_filters('flms_no_purchase_option', $no_products_text, array($post_id,$course_version));
	}
	$return = '';
			...
			if($course_version['course_settings']['course_access'] == 'open') {
				...
				if(flms_is_module_active('woocommerce')) {
					...
					if($flms_latest_version == $version_index) {
						$product_id = '';
						$status = 'publish';
						if(isset($versions[$flms_latest_version]['version_status'])) {
							$status = $versions[$flms_latest_version]['version_status'];
						}
						if($status == 'publish') {
						...
						if($product_id != '') {
							...
							if($status == 'archived') {
								$nopurchase_text = 'This course is no longer available for purchase.';
							} else {
								echo $status;
								$nopurchase_text = 'There are no options to purchase this course.';
							}
							$nopurchase_text = apply_filters('flms_no_purchase_text', $nopurchase_text, $status);
							$noproducts = '<p class="no-purchase-option">'.$nopurchase_text.'</p>';
...
function flms_get_user_completed_courses($user_id = 0) {
	...
	global $wpdb;
	$table = FLMS_ACTIVITY_TABLE;
	$sql_query = $wpdb->prepare("SELECT * FROM $table WHERE customer_status=%s AND customer_id=%d ORDER BY completion_date DESC", 'completed', $user_id);
	$results = $wpdb->get_results( $sql_query, ARRAY_A ); 
	if(!empty($results)) {
		return $results;
	} else {
...
function flms_get_user_completed_course_list($user_id, $active_courses, $echo = false) {
	...
	$list = '<div class="my-courses-list course-list-item '.$extra_class.'">';
		...
		foreach($active_courses as $active_course) {
			...
			if(array_key_first($course_data) == $course_version) {
				...
				if(isset($course_data["$course_version"]['version_name'])) {
					$title = flms_get_the_title($course_id) .' <span class="flms-version-name">('.$course_data["$course_version"]['version_name'].')</span>';
				} else {
					$title = flms_get_the_title($course_id);
				}
			...
			//$list .= '<div class="course-list-item '.$extra_class.'">';
				...
				//$list .= '<div class="course-meta">';
					...
					$list .= '<div class="actions text-right" data-label="Actions:">';
						if(is_admin()) {
							$admin_completion_date = date('Y-m-d', strtotime($date_completed));
							...
							$list .= '<a href="#" class="profile-change-completion-date" data-entry-id="'.$active_course['id'].'" data-entry-date="'.$admin_completion_date.'">Update Completion Date</a>';
						}
						/*if(flms_is_module_active('course_certificates')) {
						...
						$review_text = apply_filters('flms_review_course_text', 'Review');
						$course_actions = '<a href="'.$permalink.'" title="Review" class="btn btn-primary my-account-course-review">'.$review_text.'</a>';
		...
		if(is_admin()) {
			...
			$list .= '<div id="completion-date-update"><div class="completion-update-container"><div class="course-info"></div><input type="date" id="new-completion-date-value" data-entry-id="" value="'.$today.'" /><div class="flms-flex"><button class="button button-primary" id="save-completion-date">Save</button><button class="button button-secondary" id="cancel-completion-date-change">Cancel</button></div></div></div>';
		}
	$list .= '</div>';
	if($echo) {
		echo $list;
	...
	$list = '<div class="my-courses-list course-list-item '.$extra_class.'">';
		...
		if(!empty($additional_columns)) {
			foreach($additional_columns as $column) {
				...
				if((is_admin() && $admin) || (!is_admin() && $frontend)) {
					$list .= '<div class="flms-font-bold flms-desktop-only';
					if(isset($column['class'])) {
						$list .= ' '.$column['class'];
					}
					$list .= '">'.$column['label'].'</div>';
				}
			}
		}
			//$list .= '</div>';
				...
				//$list .= '<div class="course-meta">';
					...
					if(is_admin()) {
						...
						if($show_course_materials) {
							...
							$list .= ' elipses " data-label="Course Materials:">';
					...
					if(!empty($additional_columns)) {
						foreach($additional_columns as $column) {
							...
							if((is_admin() && $admin) || (!is_admin() && $frontend)) {
								...
								$list .= '" data-label="'.$column['label'].'">';
									if(isset($column['callback'])) {
										if(function_exists($column['callback'])) {	
	...
	if ($courses) {
		foreach ( $courses as $course ) {
			...
			if(is_array($versions)) {
				krsort($versions);
				foreach($versions as $k => $v) {
					//$results[get_the_ID()] = flms_get_the_title();
					...
					//$results[$course_identifier] = strip_tags(flms_get_the_title($course_id) ." ($version_name) [ID: $course_id]");
					$results[$course_identifier] = flms_get_the_title($course_id) ." ($version_name)";
				}
			}
		}
		wp_reset_postdata(); 	
...
function flms_get_certificate_select_box() {
	...
	if ($certificates) {
		foreach ( $certificates as $certificate ) {
			$results[$certificate->ID] = flms_get_the_title($certificate);
		}
		wp_reset_postdata(); 	
	}
	return $results;
...
function flms_course_navigation() {
	...
	$previous_arrow = apply_filters('flms_course_navigation_previous_arrow', '&laquo;&nbsp;', $post_id, $post_type);
	$next_arrow = apply_filters('flms_course_navigation_next_arrow', '&nbsp;&raquo;', $post_id, $post_type);
	echo '<div class="flms-course-navigation flms-course-'.$flms_course_id.' flms-post-'.$post_id.'">';
	switch($post_type) {
		case 'flms-courses': 
		...
		case 'flms-lessons': 
			...
			if($prev_index >= 0) {
				if (array_key_exists($keys[$prev_index], $course_steps)) {
					...
					if (array_key_exists($last_item_in_previous_lesson, $course_steps[$prev_key])) {
						...
						echo '<a href="'.get_permalink($prev).'" class="button button-primary">'.$previous_arrow.flms_get_the_title($prev, 'flms-course-navigation').'</a>';
					} else {
						echo '<a href="'.get_permalink($prev_key).'" class="button button-primary">'.$previous_arrow.flms_get_the_title($prev_key, 'flms-course-navigation').'</a>';
					}
				}
			} else {
				echo '<a href="'.trailingslashit($course->get_course_version_permalink($flms_active_version)).'" class="button button-primary">'.$previous_arrow.flms_get_the_title($course_id, 'flms-course-navigation').'</a>';
			}
			...
			if(flms_is_step_complete($steps_completed, $post_id)) {
				...
				//show next
				echo '<a href="'.get_permalink($next).'" class="button button-primary">'.flms_get_the_title($next, 'flms-course-navigation').$next_arrow.'</a>';
			} else {
				//needs completing
		...
		case 'flms-topics':
			...
			if (array_key_exists($lesson_id, $course_steps)) {
				...
				if($index !== false && $index > 0 ) {
					...
					echo '<a href="'.get_permalink($lesson_id).'" class="button button-primary">'.$previous_arrow.flms_get_the_title($lesson_id, 'flms-course-navigation').'</a>';
				}
			...
			if (array_key_exists($post_id, $course_steps)) {
				...
				if($next_index < count($course_steps)) {
					if (array_key_exists($keys[$next_index], $course_steps)) {
						$next = $keys[$next_index];
						//echo '<a href="'.get_permalink($next_key).'" class="button button-primary">'.flms_get_the_title($next_key).'&nbsp;&raquo;</a>';
					}
				} else {
					$next = $course_id;
					//last step in course
					//echo '<a href="'.get_permalink($course_id).'" class="button button-primary">'.flms_get_the_title($course_id).'&nbsp;&raquo;</a>';
				}
				//for testing
				//flms_complete_lesson_button($post_id, $next);
				echo '<a href="'.trailingslashit(get_permalink($next)).'" class="button button-primary">'.flms_get_the_title($next, 'flms-course-navigation').$next_arrow.'</a>';
			} else {
				//lesson exam
				$parent_id = flms_get_exam_version_parent($post_id);
				if (array_key_exists($parent_id, $course_steps)) {
					...
					if($index !== false && $index > 0 ) {
						...
						//no previous step in lesson
						echo '<a href="'.get_permalink($parent_id).'" class="button button-primary">'.$previous_arrow.flms_get_the_title($parent_id, 'flms-course-navigation').'</a>';
					}
					...
					if($index !== false && $index < count($course_steps[$parent_id])-1) {
						...
						//echo '<a href="'.get_permalink($next).'" class="button button-primary">'.flms_get_the_title($next).'&nbsp;&raquo;</a>';
					} else {
...
function flms_complete_lesson_button($post_id, $next, $echo = true) {
	...
	if(flms_is_step_complete($flms_user_activity, $post_id)) {
		...
		if($next > 0) {
			$return .= '<a href="'.get_permalink($next).'" class="button button-primary">'.flms_get_the_title($next).'&nbsp;&raquo;</a>';
		}
	} else {
...
function flms_lesson_is_complete($post_id, $echo = true) {
	...
	$return = '<div class="flms-completed-step flms-step-type-'.$post_type.' button button-primary">'.flms_step_complete_checkbox($post_id).$label.'</div>';
	if($echo) {
		echo $return;
	} else {
		return $return;
...
function flms_get_taxonomy_id($tax_name_or_id, $taxonomy) {
	...
	if($cat == false){
		...
		if(!is_wp_error( $cat ) ) {
			...
			//category could not be created, return the error
			$cat_id = $cat;
		}
	} else{ 
		//category already exists, get ID
...
function flms_get_customer_past_course_completions($course_id, $course_version, $user_id = 0) {
	global $wpdb;
	if($user_id == 0) {
		global $current_user;
		$user_id = $current_user->ID;
	...
	$table = FLMS_ACTIVITY_TABLE;
	$sql_query = $wpdb->prepare("SELECT * FROM $table WHERE course_id=%d AND course_version=%d AND customer_id=%d ORDER BY id DESC", $course_id, $course_version, $user_id);
	$results = $wpdb->get_results( $sql_query, ARRAY_A ); 
	$completions = array();
	if(!empty($results)) {
		foreach($results as $result) {
...
function flms_my_courses_output($course_id, $layout = 'list') {
	...
	$output = '<div class="flms-course flms-course-output layout-'.$layout.'">';
		...
		if($show_image) {
			if($layout == 'list') {
				if(has_post_thumbnail( $course_id )) {
					...
					$img_output = '<div class="course-image" style="width: '.$img_width.'">';
						$img_output .= '<a href="'.get_permalink($course_id).'" class="flms-course-title">'.get_the_post_thumbnail($course_id).'</a>';
					$img_output .= '</div>';
				} else {
					$img_output = '';
				}
			} else {
				$img_output = '<div class="course-image">';
					$img_output .= '<a href="'.get_permalink($course_id).'" class="flms-course-title">'.get_the_post_thumbnail($course_id).'</a>';
				$img_output .= '</div>';
			}
			$output .= apply_filters('flms_course_image_output', $img_output, $course_id);
		}
		//}
		$output .= '<div class="course-info">';	
			$output .= '<a href="'.get_permalink($course_id).'" class="flms-course-title">'.apply_filters( 'flms_page_title',$course->get_course_version_name($flms_latest_version)).'</a>';
			$output .= apply_filters('flms_course_list_after_course_title', '', $course_id);
			//$output .= '<div class="flms-course-title">'.flms_get_the_title($course_id).'</div>';
		...
		$output .= '<div class="product-data">';
			...
			$course_label = apply_filters('flms_view_course_button_text', $course_label);
	...
	$return .= '<form id="flms-course-filters" class="flms-course-filters flms-flex no-wrap flex-column align-left '.$force_height.'" action="'.get_bloginfo('url').'/courses">';
		$return .= '<div class="flms-filter-section full-flex">';
			...
			$return .= '<div class="flms-filter-options flms-flex flex-column full-flex">';
				$return .= '<div class="flms-label flex-1">'.apply_filters('flms_searchterm_label','Key words').'</div>';
				$course_term = '';
				...
				$return .= '<div class="flms-flex full-flex course-credits-filters">';
					foreach($flms_settings['course_taxonomies'] as $taxonomy_name => $options) {
						if($options['filter-status'] == 'show') {
							$hidden_terms = apply_filters("flms_hide_{$taxonomy_name}_terms", array());
...
function flms_get_course_version_permalink($course_id, $course_version = 1) {
	global $flms_active_version;
	$course = new FLMS_Course($course_id);
	$flms_active_version = $course_version;
...
function flms_alert($message, $dismissable = false, $extra_classes = '') {
	$return = '<p class="flms-alert flms-primary flms-secondary-bg flms-secondary-border flms-flex '.$extra_classes.'">';
...
function flms_seconds_to_time($seconds, $show_seconds = true) {
...
function flms_get_site_user_count() {
	global $wpdb;
	$total_users = $wpdb->get_var("SELECT COUNT(ID) FROM {$wpdb->users}");
	return $total_users;
}

function flms_export_user_data($user) {
	$fields = array();
	$fields[] = $user->ID;
...
function flms_timestamp_difference_display($startTimestamp, $endTimestamp) {
	...
	$remainingSecondsAfterDays = $diffInSeconds % (24 * 60 * 60);
	$hours = floor($remainingSecondsAfterDays / (60 * 60));
	...
	if ($seconds > 0 || empty($output)) { // Always show seconds if nothing else is shown
...
function flms_natural_language_join(array $list, $conjunction = 'and') {
	$last = array_pop($list);
	if ($list) {
