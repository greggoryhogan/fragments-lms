...
final class FLMS {
    ...
    private function includes() {
		...
		if(flms_is_module_active('woocommerce') && class_exists( 'woocommerce' )) {
	...
	public function register_frontend_scripts() {
		...
		if(isset($post)) {
			...
			if($post->post_type == 'flms-exams') {
				...
				$limit = 10;
				if(isset($exam_settings['questions_per_page'])) {
	...
	private function modules() {
		...
		$this->define('FLMS_USER_ACTIVITY_TABLE', $wpdb->base_prefix.'flms_user_activity');
	...
	public function flms_init_db_tables() {
		...
		//Course Activity
		$tablename = FLMS_USER_ACTIVITY_TABLE;
		$sql = "CREATE TABLE $tablename (
			id bigint(20)  unsigned NOT NULL auto_increment,
			customer_id bigint(20) ,
			post_id varchar(250),
			post_version varchar(250),
			post_action varchar(250),
