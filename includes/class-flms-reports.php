...
class FLMS_Reports {
        ...
        ?><div class="wrap">
            ...
            <div class="page-reports">
                <div class="flex space-between align-flex-start reports-options">
                    <form id="reports-settings" action="<?php echo admin_url('admin.php?page=flms-reports'); ?>">
                        <div>
                            ...
                            $report_types = apply_filters('flms_report_types', $report_types);
                ...
                foreach( $posts as $post ) {
                    echo flms_get_the_title($post->ID).'<br>';
        ...
        switch($type) {
            ...
            case 'course_credits': 
                ...
                if(!empty($entries)) {
                    ...
                    $start_query_date = date('Y-m-d 0:00:00',strtotime($start_date)); //2022-09-29 19:43:24
		            $end_query_date = date('Y-m-d 23:59:59',strtotime($end_date)); //2022-09-29 19:43:24
                    ...
                    if(!empty($results)) {
                        ...
                        foreach($results as $result) {
                            ...
                            $user_link = '<a href="'.get_edit_user_link($data['id']).'" class="edit-user-link" target="_blank"><span></span></a>';
                            $user_email_data = '<div class="flms-flex align-center gap-sm">'.$user_link.$data['email'].'</div>';
                            if($data['credits'] > 0) {
                                $has_results = true;
                                $response .= '<tr>';
                                    $response .= '<td data-title="Last name">'.$data['last_name'].'</td>';
                                    $response .= '<td data-title="First name">'.$data['first_name'].'</td>';
                                    $response .= '<td data-title="Email">'.$user_email_data.'</td>';
            ...
            case 'royalties':
                ...
                $amount = (float) get_term_meta($term->term_id, 'flms_royalty_amount', true);
                ...
                if($course_query->have_posts()) {
                    ...
                    $order_args = apply_filters('flms_royalty_reporting_order_args', $order_args);
                ...
                break;
            default:
                $report_type_name = apply_filters('flms_custom_report_type_name', 'Custom Report', $type);
                $response = apply_filters('flms_custom_report_response', '', $type, $report_data);
                $default_report_information = '<div data-report-type="'.$type.'"><span>Report type:</span> '.$report_type_name.'</div>';
                $report_information = apply_filters('flms_custom_report_information', $default_report_information, $type, $report_data);
                break;
        }
		unset($_POST['action']);
    ...
    public function maybe_resort_royalties($course_royalties) {
        if(apply_filters('flms_sort_royalty_report_by_title', false)) {
            $titles = array();
            foreach($course_royalties as $course_id => $course_orders) {
                $titles[$course_id] = flms_get_the_title($course_id);
            }
            // Sort courses by title while keeping course ID as the key
            uasort($course_royalties, function ($a, $b) use ($titles, $course_royalties) {
                $idA = array_search($a, $course_royalties); // Get course ID for $a
                $idB = array_search($b, $course_royalties); // Get course ID for $b

                if ($idA === false || $idB === false) {
                    return 0; // Fallback to avoid unexpected errors
                }

                return strcmp($titles[$idA], $titles[$idB]);
            });
        }
        return $course_royalties;
    }
    ...
    public function export_report($data) {
            ...
            foreach($data as $k => $v) {
                if(is_array($v)) {
                    ...
                    foreach($v as $index => $value) {
                        $sanitized_array[$index] = sanitize_text_field($value);
                    }
                    $sanitized_data[$k] = $sanitized_array;
                } else {
                    $sanitized_data[$k] = sanitize_text_field($v);
                }
        ...
        switch($type) {
            ...
            case 'royalties': 
                ...
                break;
            default:
                return apply_filters('flms_custom_report_export_response', false, $type, $data);
                break;
    ...
    public function get_customer_credit_data($result, $credit_type, $export = false) {
        ...
        $continue = true;
        if($customer !== false) {
            $user_has_license = get_user_meta( $customer_id, "flms_has-license-$credit_type", true);
            //$user_license_number = get_user_meta( $customer_id, "flms_license-$credit_type", true);
            if($user_has_license == '') {
                $continue = false;
            }
        } else {
        ...
        $continue = apply_filters('flms_user_included_in_credit_reporting', $continue, $customer_id, $customer);
        if(!$continue) {
            return false;
        }
        if($credit_value > 0 && $continue ) {
            $course = new FLMS_Course($course_id);
            
            $course_title = $course->get_course_version_name($course_version);
            $course_link = '<a href="'.get_edit_post_link($course_id).'&set-course-version='.$course_version.'" title="'.strip_tags($course_title).'" target="_blank">'.$course_title.'</a>';
            //if ( $customer !== false ) {
                $data['id'] = $customer_id;
