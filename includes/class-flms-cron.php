...
class FLMS_Cron {
    ...
    public function __construct() {
        add_action('init', array($this, 'check_for_stagnamt_crons'));
	}
    ...
    public function import_courses($file, $import_action, $processed_rows = 0, $field_indexes = array(), $errors = array(), $user_id = 0) {
        ...
        $course_import_count = get_option( 'flms_courses_import_ct' );
        //$course_import_count = false; //for testing
        if($course_import_count === false) {
            $course_import_count = 0;
            if (($handle = fopen($file, 'r')) !== false) {
                $deliminator = flms_detect_csv_elimiter($file);
                while (($data = fgetcsv($handle, 0, $deliminator)) !== false) {
                    ++$course_import_count;
                }
                update_option('flms_courses_import_ct', $course_import_count);
                fclose($handle);
            }
        } 
        
        $itracker = $processed_rows;
        $rows = 0;
        if (($handle = fopen($file, 'r')) !== false) {
            $deliminator = flms_detect_csv_elimiter($file);
            ...
            while (($data = fgetcsv($handle, 0, $deliminator)) !== false) {
                ...
                // Skip rows that are not in the current batch
                if ($rows <= $processed_rows) {
                    continue;
                }
                
                // Stop processing after reaching the max rows for this batch
                if ($rows > $max_rows) {
                    break;
                }
                ...
                if((!is_numeric($course_id) && $import_action == 'update') || (!is_numeric($course_id) && $rows == 1 && $import_action == 'insert'))  {
                    $errors[] = 'Course ID is not a number. Likely reading header of import file.';
                    $continue = false;
                } else {
                    $question_args = array();
                    if($field_indexes['Title'] != -2) {
                        $question_args['post_title'] = apply_filters('flms_import_content_title', $data[$field_indexes['Title']]);
                    }
                ...
                if($continue) {
                    if($field_indexes['Version'] == -2) {
                        ...
                        if(flms_is_module_active('course_certificates')) {
                            if($field_indexes['Course Certificates'] != -2) {
                                $certificates = explode('|',$data[$field_indexes['Course Certificates']]);
                                $certificate_ids = array();
                                if(!empty($certificates)) {
                                    foreach($certificates as $certificate) {
                                        $id = flms_get_post_id($certificate, 'flms-certificates');
                                        if($id !== false) {
                                            $certificate_ids[] = $id;
                                        }
                                    }
                        ...
                        if(flms_is_module_active('course_credits')) {
                            ...
                            foreach($credit_fields as $credit_field) {
                                ...
                                if($field_indexes["$label Credits"] != -2) {
                                    $versioned_content["$version"]['course_credits']["$credit_field"] = floatval($data[$field_indexes["$label Credits"]]);
                                }
                                if(flms_is_module_active('course_numbers')) {
                                        ...
                                        if($material != '') {
                                            ...
                                            if(isset($material_data[0])) {
                                                if($material_data[0] != '') {
                                                    ...
                                                    $import_course_materials = apply_filters('flms_import_course_materials', false);
                        ...
                        if(flms_is_module_active('course_taxonomies')) {
                            ...
                            foreach($tax_fields as $tax_field) {
                                ...
                                if($field_indexes[$label] != -2) {
                                    ...
                                    $values = apply_filters('flms_import_course_taxonomies', $values, $slug);
                                    $insert_values = array();
                                    if(!empty($values)) {
                        ...
                        if(flms_is_module_active('woocommerce')) {
                            ...
                            $force_new_product_creation = apply_filters('flms_generate_new_products_on_import', false);
        ...
        if($itracker >= $course_import_count) {
            //clear cron
            //wp_clear_scheduled_hook('flms_import_courses');
            //delete option
            delete_option('flms_courses_import');
            //delete transient
            //delete_transient('flms_course_import_data');
            delete_option('flms_courses_import_ct');
            //notify user
            ...
            $cron_data = array(
                ...
                'processed_rows' => $itracker,
                'field_indexes' => $field_indexes,
                'errors' => $errors,
                'user_id' => $user_id
            );
            //if (!wp_next_scheduled('flms_import_courses', $cron_data)) {
                wp_schedule_single_event( time() + 30, 'flms_import_courses', $cron_data );
                $max_rows_reached = true;
                $option_data = array(
                    'processed' => $itracker,
                    'total' => $course_import_count
                );
                update_option('flms_courses_import', maybe_serialize($option_data), false);
            //}
    ...
    public function check_for_stagnamt_crons() {
        /*$course_import_process = get_option('flms_course_import');
        if($course_import_process !== false) {
    ...
    public function import_exams($file, $import_action, $processed_rows = 0, $field_indexes = array(), $errors = array(), $user_id = 0) {
        ...
        $course_import_count = get_option( 'flms_exams_import_ct' );
        //$course_import_count = false; //for testing
        if($course_import_count === false) {
            $course_import_count = 0;
            if (($handle = fopen($file, 'r')) !== false) {
                ...
                update_option('flms_exams_import_ct', $course_import_count);
                fclose($handle);
            }
        } 
                    ...
                    if($field_indexes['Associated Content'] != -2) {
                        ...
                        if($associated_content == '') {
                            ...
                            if($associated_content_post_type == '') {
                                ...
                                $version = $data[$field_indexes['Version']];
                                $associated_content_args = apply_filters('flms_associated_content_args', array('post_type' => $associated_content_post_type, 'post__in' => array($associated_content)));
                                $posts = get_posts($associated_content_args);
                                if(!empty($posts)) {
                                    foreach($posts as $flms_post) {
                                        ...
                                        if(!isset($versioned_content["$version"])) {
                                            ...
                                            if((!is_numeric($exam_id) && $import_action == 'update') || (!is_numeric($exam_id) && $rows == 1 && $import_action == 'insert'))  {
                                                $errors[] = 'Exam ID is not a number. Likely reading header of import file.';
                                                $continue = false;
                                            } else {
                                                if($import_action == 'insert') {
        ...
        if($itracker >= $course_import_count) {
            ...
            update_option('flms_exams_import', maybe_serialize($option_data), false);
        }
    ...
    public function import_questions($file, $import_action, $processed_rows = 0, $field_indexes = array(), $errors = array(), $user_id = 0) {
        ...
        $course_import_count = get_option( 'flms_questions_import_ct' );
        //$course_import_count = false; //for testing
        if($course_import_count === false) {
            $course_import_count = 0;
            if (($handle = fopen($file, 'r')) !== false) {
                ...
                update_option('flms_questions_import_ct', $course_import_count);
                fclose($handle);
            }
        } 
        ...
        if (($handle = fopen($file, 'r')) !== false) {
            ...
            while (($data = fgetcsv($handle, 0, $deliminator)) !== false) {
                ...
                if($field_indexes['Menu Order'] != -2) {
                    $question_args['menu_order'] = $data[$field_indexes['Menu Order']];
                }


                //check for ID
                if((!is_numeric($question_id) && $import_action == 'update') || (!is_numeric($question_id) && $rows == 1 && $import_action == 'insert'))  {
                    $errors[] = 'Question ID is not a number. Likely reading header of import file.';
                    $continue = false;
                } else {
                    //insert or update title, content
                    if($import_action == 'insert') {
                    ...
                    if($field_indexes['Category'] != -2) {
                        ...
                        if(!empty($categories)) {
                            foreach($categories as $category) {
                                $category_to_process = apply_filters('flms_category_import_name', $category);
                                $cat  = get_term_by('name', $category_to_process, $taxonomy);
                                if($cat === false){
                                    //cateogry not exist create it 
                                    $cat = wp_insert_term($category_to_process, $taxonomy);
                                    if(!is_wp_error( $cat )) {
                                        $cat_id = $cat['term_id'];
                                    } else {
                                        $errors[] = "Could not create category $category_to_process.";
                                    }
                                } else{ 
                    ...
                    if($field_indexes['Answer'] != -2 && $field_indexes['Options'] != -2) {
                        ...
                        switch($question_type) {
                            ...
                            case 'multiple-choice':
                                $question_meta = array();
                                $options = array_map('trim', explode('|',$question_options));
                                $answers = array_map('trim', explode('|',$question_answers));
                                $found_correct = false;
                                foreach($options as $option) {
                                ...
                                if(apply_filters('flms_shuffle_questions_on_import', false)) {
                                    shuffle($question_meta);
                                }
                                update_post_meta($question_id,'flms_question_answer', $question_meta );
                                if(!$found_correct) {
                                    $errors[] = "Could not find correct answer for question $question_id.";
                                }
                                break;
                            case 'free-choice':
        ...
        if($itracker >= $course_import_count) {
            ...
            update_option('flms_questions_import', maybe_serialize($option_data), false);
        }
    ...
    public function cron_notices() {
        ...
        if ( 'is_complete' == get_transient( 'flms_export_complete' ) ) {
            ?>
            <div class="notice notice-success is-dismissible">
                <p><?php _e( FLMS_PLUGIN_NAME .' export complete!', 'flms' ); ?></p>
            </div>
            <?php
            delete_transient( 'flms_export_complete' );
        } 
    }
    ...
    public function export_complete_notification($user_id, $type, $errors = array()) {
        $to = '';
        if($user_id > 0) {
            $user = get_user_by( 'id', $user_id );
            if($user !== false) {
        ...
        if($to != '') {
            if($type == 'flms-users') {
                $message = "Your user export is complete. The file can be download in the 'Export History' tab in the ".FLMS_PLUGIN_NAME .'  exporter.';
            }
            $message = apply_filters('flms_export_complete_notification_message', $message, $type);
            $show_errors = apply_filters('flms_export_notication_show_errors', true);
            if($show_errors && !empty($errors)) {
                $message .= '<p><strong>There were errors during the export:</strong></p>';
                $message .= '<p>'.implode('<br>',$errors).'</p>';
            }
            $headers = array('Content-Type: text/html; charset=UTF-8');
            wp_mail($to, FLMS_PLUGIN_NAME .' export complete!',$message, $headers);
        }
    ...
    public function plugin_cleanup() {
        if ( !wp_next_scheduled( 'flms_cleanup' ) ) {
            wp_schedule_event( current_time( 'timestamp' ), 'daily', 'flms_cleanup');
        }
    }

    public function daily_cleanup() {

        do_action('flms_before_daily_cleanup');

        global $wpdb;

        //remove course metadata 
        $trashed_posts = get_posts( array( 'post_status' => 'trash', 'post_type' => 'flms-courses', 'posts_per_page' => -1 ) );
        if(!empty($trashed_posts)) {
            $table = FLMS_COURSE_QUERY_TABLE;
            foreach ( $trashed_posts as $post ) {
                //remove old data
                /*if(get_post_type() == 'flms-courses') {
                    $product_id = get_post_meta($post->ID, 'flms_woocommerce_product_id', true);
                    $product = wc_get_product($product_id);
                    $product->delete();   
                }*/
                $wpdb->delete( $table, array( 'course_id' => $post->ID ) );
            }
        }

        //Remove entries from course activity where user id does not exist (ie user was deleted)
        $table = FLMS_ACTIVITY_TABLE;
        $sql_query = "SELECT DISTINCT ca.id FROM $table ca LEFT JOIN wp_users u ON ca.customer_id = u.ID WHERE u.ID IS NULL";
        $results = $wpdb->get_results( $sql_query, ARRAY_A ); 
        if(!empty($results)) {
            foreach($results as $result) {
                //delete from activity table
                $wpdb->delete( FLMS_ACTIVITY_TABLE, array( 'id' => $result['id'] ) );
                //delete from credit reporting table
                $wpdb->delete( FLMS_REPORTING_TABLE, array( 'entry_id' => $result['id'] ) );
            }
            
        }

        //remove user activity entries for deleted users
        $table = FLMS_USER_ACTIVITY_TABLE;
        $sql_query = "SELECT DISTINCT ca.id FROM $table ca LEFT JOIN wp_users u ON ca.customer_id = u.ID WHERE u.ID IS NULL";
        $results = $wpdb->get_results( $sql_query, ARRAY_A ); 
        if(!empty($results)) {
            foreach($results as $result) {
                $wpdb->delete( FLMS_USER_ACTIVITY_TABLE, array( 'id' => $result['id'] ) );
            }
            
        }

        do_action('flms_after_daily_cleanup');
    ...
    public function export_user_data($filename, $processed_users = 0, $user_id = 0) {
        ...
        //$offset = $processed_users + $iteration;
        $args = array(
            'number' => $iteration,
            'offset' => $processed_users,
            'orderby' => 'ID',
        );
        ...
        if($processed_users < $total_users) {
            $percent = floor(($processed_users / $total_users) * 100).'%';
            update_option('flms_user_export', $percent);
            $cron_data = array(
                'filename' => $filename,
                'processed_users' => $processed_users,
                'user_id' => $user_id
            );
            wp_schedule_single_event( time() , 'flms_export_user_data', $cron_data );
        } else {
            delete_option('flms_user_export');
            //clear cron
            wp_clear_scheduled_hook('flms_export_userdata');
            //notify user
            $this->export_complete_notification($user_id, 'flms-users', array()) ;
            set_transient( 'flms_export_complete', 'is_complete' );
